...
- **Image attachment** (#2584/#2587): `ContentBlock::ImageUrl`, multimodal chat requests, base64 encoding, + exhaustive match arms across 15 files. Deferred by blissito — changes the request shape, needs multimodal endpoint testing.
...
cargo test -p ghosty-tui -- --test-threads=4
...
gh issue view 2600 --repo blissito/ghostycode  # re-read release checklist
```
...
- [ ] Full `cargo test --workspace` green — ghosty-tui validated; 1 environment-only Landlock test may fail on macOS
- [ ] `npm test` in `npm/ghosty`
...
2. **DeepSeek naming**: The branch and committed code use "Ghosty Code" naming throughout. Do not imply DeepSeek is deprecated.
...
Generated by deepseek-v4-pro in Ghosty Code v0.8.51 pre-release triage.
