...
use ghosty_config::{CliRuntimeOverrides, ConfigStore};
...
async fn tool_handler(
    ...
    match runtime
        .invoke_tool(req.call, ghosty_execpolicy::AskForApproval::OnRequest, &cwd)
        .await
    {
        Ok(value) => Json(value),
...
fn resolve_auth_token(options: &AppServerOptions) -> Result<Option<String>> {
    ...
    if options.auth_token.is_some() {
        ...
        eprintln!("  Pass --auth-token or set GHOSTY_APP_SERVER_TOKEN for a stable token.");
    }
    Ok(Some(token))
}
