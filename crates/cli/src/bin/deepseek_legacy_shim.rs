...
//!
//! Forwards argv to the `ghosty` dispatcher and prints a one-line
...
fn main() {
    eprintln!(
        "warning: `deepseek` is deprecated; run `ghosty` instead. \
    ...
    let args: Vec<String> = env::args_os()
        ...
        .collect();
    let status = match Command::new("ghosty").args(&args).status() {
        Ok(s) => s,
        Err(e) => {
            eprintln!(
                "error: failed to spawn `ghosty`: {e}. Is it on PATH? \
                 Install with `cargo install ghosty-cli` or via npm/Homebrew."
            );
