...
")]
    ...
    #[command(after_help = "\
Examples:
  ghosty swebench run --instance-id django__django-12345 --issue-file issue.md
  ghosty swebench export --instance-id django__django-12345 --predictions-path all_preds.jsonl
...
")]
    ...
    /// Run a Ghosty Code-powered code review over a git diff.
...
fn run() -> Result<()> {
    ...
    match command {
        ...
        Some(Commands::Completion { shell }) => {
            ...
            generate(shell, &mut cmd, "ghosty", &mut io::stdout());
            Ok(())
        }
...
fn exit_with_tui_status(status: std::process::ExitStatus) -> Result<()> {
    match status.code() {
        ...
        None => bail!("ghosty-tui terminated by signal"),
    }
}
...
\n\
The `ghosty` dispatcher found a `ghosty-tui` file, but the OS refused \
to execute it. Common fixes:\n\
  - Reinstall with `npm install -g ghosty`, or run `ghosty update`.\n\
  - On Windows, run `where ghosty` and `where ghosty-tui`; both should \
come from the same install directory.\n\
  - If you downloaded release assets manually, keep both `ghosty` and \
`ghosty-tui` binaries together and make sure the TUI binary is executable.\n\
  - Set DEEPSEEK_TUI_BIN to the absolute path of a working `ghosty-tui` \
binary.",
        tui.display()
    )
}

/// Resolve the sibling `ghosty-tui` executable next to the running
/// dispatcher. Honours platform executable suffix (`.exe` on Windows) so
/// the npm-distributed Windows package — which ships
/// `bin/downloads/ghosty-tui.exe` — is found by `Path::exists` (#247).
///
...
fn locate_sibling_tui_binary() -> Result<PathBuf> {
    ...
    // expected name, not "ghosty-tui" on Windows.
    let expected = current.with_file_name(format!("ghosty-tui{}", std::env::consts::EXE_SUFFIX));
    bail!(
        "Companion `ghosty-tui` binary not found at {}.\n\
\n\
The `ghosty` dispatcher delegates interactive sessions to a sibling \
`ghosty-tui` binary. To fix this, install one of:\n\
  • npm:    npm install -g ghosty                (downloads both binaries)\n\
  • cargo:  cargo install ghosty-cli ghosty-tui --locked\n\
  • GitHub Releases: download BOTH `ghosty-<platform>` AND \
`ghosty-tui-<platform>` from https://github.com/blissito/ghostycode/releases/latest \
and place them in the same directory.\n\
\n\
Or set DEEPSEEK_TUI_BIN to the absolute path of an existing `ghosty-tui` binary.",
        expected.display()
    );
}

/// Return the first existing sibling-binary path under any of the names
/// `ghosty-tui` might use on this platform. Pure function to keep
...
fn sibling_tui_candidate(dispatcher: &Path) -> Option<PathBuf> {
    ...
    // on Windows.
    let primary = dispatcher.with_file_name(format!("ghosty-tui{}", std::env::consts::EXE_SUFFIX));
    if primary.is_file() {
        return Some(primary);
    }
    ...
    if cfg!(windows) {
        let suffixless = dispatcher.with_file_name("ghosty-tui");
...
mod tests {
    ...
    fn build_tui_command_allows_openai_and_forwards_provider_key() {
        ...
        assert!(
            args.windows(2)
                .any(|pair| pair == ["--workspace", "/tmp/ghosty-workspace"]),
    ...
    /// sibling `ghosty-tui.exe`, not bail out looking for an
    /// extension-less `ghosty-tui`. The candidate resolver also accepts
    ...
    fn sibling_tui_candidate_picks_platform_correct_name() {
        ...
        let target =
            dispatcher.with_file_name(format!("ghosty-tui{}", std::env::consts::EXE_SUFFIX));
    ...
    fn dispatcher_spawn_error_names_path_and_recovery_checks() {
        ...
        let message = tui_spawn_error(Path::new("C:/tools/ghosty-tui.exe"), &err);

        assert!(message.contains("C:/tools/ghosty-tui.exe"));
        assert!(message.contains("access is denied"));
        assert!(message.contains("where ghosty"));
    ...
    fn sibling_tui_candidate_windows_falls_back_to_suffixless() {
        let dir = tempfile::TempDir::new().expect("tempdir");
        let dispatcher = dir.path().join("ghosty.exe");
        std::fs::write(&dispatcher, b"").unwrap();
        ...
        let found = sibling_tui_candidate(&dispatcher)
            .expect("Windows fallback must locate suffixless ghosty-tui");
