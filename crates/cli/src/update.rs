...
pub(crate) fn binary_prefix_for_exe(current_exe: &Path) -> &'static str {
    let exe_name = current_exe
        ...
        .unwrap_or("ghosty");
    if exe_name.contains("ghosty-tui") {
        "ghosty-tui"
    } else {
        "ghosty"
    }
...
fn release_from_mirror_base_url(
    ...
    for prefix in ["ghosty", "ghosty-tui"] {
...
fn fetch_latest_beta_release_from_url(url: &str, proxy: Option<&Proxy>) -> Result<Release> {
    ...
    // GitHub caps this endpoint at 100 releases per page. Ghosty Code uses the
...
mod tests {
    ...
    fn test_binary_prefix_detection() {
        // TUI binary should use ghosty-tui prefix
        assert_eq!(binary_prefix_for_exe(Path::new("ghosty-tui")), "ghosty-tui");
        assert_eq!(
            binary_prefix_for_exe(Path::new("ghosty-tui.exe")),
            "ghosty-tui"
        );
        assert_eq!(
            binary_prefix_for_exe(Path::new("/usr/local/bin/ghosty-tui")),
            "ghosty-tui"
        );

        // Dispatcher binary should use ghosty prefix
        assert_eq!(binary_prefix_for_exe(Path::new("ghosty")), "ghosty");
        assert_eq!(binary_prefix_for_exe(Path::new("ghosty.exe")), "ghosty");
        assert_eq!(
            binary_prefix_for_exe(Path::new("/usr/local/bin/ghosty")),
            "ghosty"
        );

        // Fallback for unknown names
        assert_eq!(binary_prefix_for_exe(Path::new("other-binary")), "ghosty");
    }
    ...
    fn test_release_asset_stem_for_supported_platforms() {
        let cases = [
            ...
            ("ghosty-tui", "macos", "aarch64", "ghosty-tui-macos-arm64"),
            ("ghosty-tui", "linux", "x86_64", "ghosty-tui-linux-x64"),
        ];
    ...
    fn update_targets_include_existing_sibling_tui_for_dispatcher() {
        ...
        assert!(targets[0].asset_stem.starts_with("ghosty-"));
        assert!(targets[1].asset_stem.starts_with("ghosty-tui-"));
    }
...
";
    ...
    /// Mocked GitHub release payload covering both the dispatcher (`ghosty`)
    /// and the legacy TUI (`ghosty-tui`) binaries across our published
    ...
    fn mocked_release() -> Release {
        let json = r#"{
          ...
          "assets": [
            ...
            { "name": "ghosty-tui-linux-x64",      "browser_download_url": "https://example.invalid/ghosty-tui-linux-x64" },
            { "name": "ghosty-tui-macos-x64",      "browser_download_url": "https://example.invalid/ghosty-tui-macos-x64" },
            { "name": "ghosty-tui-macos-arm64",    "browser_download_url": "https://example.invalid/ghosty-tui-macos-arm64" },
            { "name": "ghosty-tui-windows-x64.exe","browser_download_url": "https://example.invalid/ghosty-tui-windows-x64.exe" }
          ]
        }"#;
    ...
    fn mocked_release_selects_tui_asset_when_tui_binary_invokes_update() {
        ...
        assert_eq!(asset.name, "ghosty-tui-macos-arm64");
    }
    ...
    fn mirror_release_uses_base_url_and_platform_assets() {
        ...
        let tui = select_platform_asset(&release, "ghosty-tui-linux-x64").expect("tui asset");
        assert_eq!(
            tui.browser_download_url,
            "https://mirror.example/releases/v0.8.36/ghosty-tui-linux-x64"
        );
    }
    ...
    fn mirror_release_uses_windows_exe_asset_names() {
        ...
        assert!(
            select_platform_asset(&release, "ghosty-tui-windows-x64")
                .is_some_and(|asset| asset.name == "ghosty-tui-windows-x64.exe")
        );
    }
