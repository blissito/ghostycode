...
pub use ghosty_execpolicy::ToolAskRule;
use ghosty_execpolicy::{ExecPolicyEngine, PermissionAction, Ruleset};
use ghosty_secrets::SecretSource;
pub use ghosty_secrets::Secrets;
...
pub fn is_upstream_auth_header(name: &str) -> bool {
    ...
    // generated by Ghosty itself. `auth_mode = "none"` is an endpoint
...
pub struct ProviderConfigToml {
    ...
    /// Ghosty-owned xAI OAuth generation selected by config. The value is a
    /// validated basename under `$GHOSTY_HOME/credentials`, never an
    /// arbitrary path.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub oauth_credential_generation: Option<String>,
    /// Preserve provider fields introduced by newer Ghosty versions and by
...
pub struct ConfigToml {
    ...
    /// and `ghosty-tui` can share a single config file.
    pub api_key: Option<String>,
    ...
    /// Native tool catalog controls shared with `ghosty-tui`.
    #[serde(default)]
...
impl ConfigToml {
    ...
    /// Merge safe project-level overrides from `$WORKSPACE/.ghosty/config.toml`
    ...
    pub fn resolve_runtime_options_with_secrets(
        ...
        let (provider, provider_source) = if let Some(provider) = cli.provider {
            ...
            (
                provider,
                ProviderSource::Env(env.provider_source.unwrap_or("GHOSTY_PROVIDER")),
            )
        } else {
        ...
        // Root `default_text_model` is the key `ghosty model set` writes and
        ...
        // and therefore `ghosty model resolve`, reported a provider default.
        //
        ...
        // `GHOSTY_TELEMETRY=0` are run-scoped kill switches: they must stop
...
/// `GHOSTY_TELEMETRY=0`, `telemetry = false`, and a recorded decline all
/// stop the session long before an endpoint is read.
///
/// An explicit value — `GHOSTY_TELEMETRY_ENDPOINT` or `telemetry_endpoint` in
...
/// `$GHOSTY_HOME/telemetry/dryrun.jsonl`, and no HTTP client is constructed.
pub const DEFAULT_TELEMETRY_ENDPOINT: &str = "https://telemetry.ghosty.net/v1/telemetry";
...
pub const CLI_API_KEY_ENV: &str = "GHOSTY_CLI_API_KEY";

/// Source marker paired with [`CLI_API_KEY_ENV`] on the CLI-to-TUI boundary.
pub const CLI_API_KEY_SOURCE_ENV: &str = "GHOSTY_CLI_API_KEY_SOURCE";
...
///
/// Private to the `ghosty` → `ghosty-tui` hop, in the same spirit as
/// [`CLI_API_KEY_SOURCE_ENV`]. Set to `1`/`0` on every delegated run.
pub const TELEMETRY_FLOOR_ENV: &str = "GHOSTY_TELEMETRY_FLOOR";
...
/// default, and the dispatcher forwards a resolved `GHOSTY_TELEMETRY=false`
...
///
/// With no statement (a directly launched `ghosty-tui`) the raw environment
...
pub fn telemetry_floor_in_force() -> bool {
    ...
    let Ok(raw) =
        std::env::var("GHOSTY_TELEMETRY").or_else(|_| std::env::var("DEEPSEEK_TELEMETRY"))
    else {
        return false;
    };
...
pub enum TelemetrySource {
    ...
    Cli,
    /// `GHOSTY_TELEMETRY`/`DEEPSEEK_TELEMETRY`, including the dispatcher's
...
fn read_telemetry_env() -> (Option<bool>, bool) {
    let Some(raw) = std::env::var("GHOSTY_TELEMETRY")
    ...
    match parse_bool(&raw) {
        ...
        Err(_) => {
            tracing::warn!(
                "Invalid GHOSTY_TELEMETRY/DEEPSEEK_TELEMETRY value '{raw}'; expected one of \
...
///
/// Checks `$WORKSPACE/.ghosty/config.toml` first, falling back to
...
pub fn load_project_config_outcome(workspace: &Path) -> ProjectConfigOutcome {
    for dir in [GHOSTY_APP_DIR, LEGACY_APP_DIR] {
...
/// A provider owner that Ghosty can identify with high confidence when an
...
pub fn provider_base_url_is_official(provider: ProviderKind, base_url: &str) -> bool {
    ...
    match provider {
        ...
        // Custom routes have no Ghosty-owned official endpoint. The
...
fn env_api_key_for_provider(provider: ProviderKind) -> Option<String> {
    ...
    ghosty_secrets::env_for(provider.as_str())
}
...
pub enum ModelSource {
    ...
    Cli,
    /// A `GHOSTY_*` environment variable.
    Env,
...
pub struct ResolvedRuntimeOptions {
    ...
    /// buffered events of a user who merely set `GHOSTY_TELEMETRY=0` for one
...
impl ConfigStore {
    ...
    /// `true` cuando el archivo no existía al cargar: el usuario viene de cero
    /// y todavía no ha elegido proveedor de forma explícita.
    #[must_use]
    pub fn is_fresh(&self) -> bool {
        self.original_raw.is_none()
    }
    ...
    pub fn append_allow_rules(&mut self, rules: &[ToolAskRule]) -> Result<usize> {
        for rule in rules {
            ...
            let Some(workspace) = rule
                .workspace
                .as_deref()
                .and_then(ghosty_execpolicy::normalize_workspace_scope)
            else {
            ...
            if let Some(path) = rule.path.as_deref()
                && ghosty_execpolicy::normalize_workspace_relative_path(path, &workspace)
...
/// backend (e.g. an [`ghosty_secrets::InMemoryKeyringStore`]) before
/// any resolver runs.
...
// ── GhostyCode state root (v0.8.44) ──────────────────────────────────
//
// v0.8.44 migrates product-owned app state from ~/.deepseek/ to
// ~/.ghosty/ while keeping ~/.deepseek/ as a compatibility fallback.
// New installs write to ~/.ghosty/. Existing installs with only
// ~/.deepseek/ continue working without data loss.

pub use ghosty_paths::{GHOSTY_APP_DIR, LEGACY_APP_DIR};

/// Resolve the primary GhostyCode home directory.
///
/// `$GHOSTY_HOME` takes precedence when set. Otherwise defaults to
/// `$HOME/.ghosty`. This is the write target for new product state.
pub fn ghosty_home() -> Result<PathBuf> {
    ghosty_paths::ghosty_home()
        .map_err(anyhow::Error::new)?
        .context("failed to resolve home directory")
}

/// Whether `$GHOSTY_HOME` is set to a non-empty value.
///
/// An explicit GhostyCode home is an isolation boundary: state/config resolvers
/// must not fall back to ambient legacy `~/.deepseek` data outside that root.
pub fn ghosty_home_is_explicit() -> bool {
    ghosty_paths::ghosty_home_is_explicit()
}
...
pub fn legacy_deepseek_home() -> Result<PathBuf> {
    ghosty_paths::legacy_deepseek_home().context("failed to resolve home directory")
}
...
/// Resolve a state subdirectory, preferring the GhostyCode root if
...
pub fn resolve_state_dir(subdir: &str) -> Result<PathBuf> {
    ...
    let explicit_ghosty_home = ghosty_home_is_explicit();
    let primary = ghosty_home()?.join(subdir);
    if explicit_ghosty_home || primary.exists() {
        return Ok(primary);
    }
...
/// Ensure a state subdirectory exists under the primary GhostyCode root,
...
/// `~/.ghosty/<subdir>` does not, the legacy directory is relocated into
...
impl StateMigration {
    pub fn user_notice(&self) -> String {
        ...
        format!(
            "Ghosty migrated legacy state ({action}):\n  {} -> {}\nYour data was preserved. Use .ghosty as the canonical state location from now on.\n{legacy_detail}\nIf no other apps use it, you can remove the legacy .deepseek tree after confirming everything looks right.",
...
pub fn ensure_state_dir_with_migration(subdir: &str) -> Result<(PathBuf, Option<StateMigration>)> {
    ensure_safe_state_subdir(subdir)?;
    let explicit_ghosty_home = ghosty_home_is_explicit();
    let dir = ghosty_home()?.join(subdir);
    let migration = if !explicit_ghosty_home {
...
/// the primary `~/.ghosty/<subdir>` location (#3240). No-op once the primary
...
fn migrate_legacy_state_dir(primary: &Path, subdir: &str) -> Result<Option<StateMigration>> {
    ...
    // The primary's parent (the ~/.ghosty root) must exist for the rename.
...
/// Resolve a project-local state subdirectory, preferring `.ghosty/`
/// when it exists, falling back to `.deepseek/` for legacy projects.
///
/// Returns `(true, path)` when the primary `.ghosty/` path is used,
...
pub fn resolve_project_state_dir(workspace: &Path, subdir: &str) -> Result<(bool, PathBuf)> {
    ...
    let primary = workspace.join(GHOSTY_APP_DIR).join(subdir);
    if primary.exists() {
...
/// Ensure a project-local state subdirectory exists under `.ghosty/`,
...
pub fn ensure_project_state_dir(workspace: &Path, subdir: &str) -> Result<PathBuf> {
    ensure_safe_state_subdir(subdir)?;
    let workspace = normalize_project_workspace(workspace)?;
    let dir = workspace.join(GHOSTY_APP_DIR).join(subdir);
    std::fs::create_dir_all(&dir)
        .with_context(|| format!("failed to create {}/", dir.display()))?;
    Ok(dir)
}
...
pub fn resolve_config_path(explicit: Option<PathBuf>) -> Result<PathBuf> {
    ...
    if let Some(path) = ghosty_paths::config_path_override().map_err(anyhow::Error::new)? {
        return normalize_config_file_path(path);
    }
    default_config_path()
}
...
/// `<repo>/.ghosty/config.toml` (or the legacy `.deepseek` layout) inside a
...
/// `.ghosty`/`.deepseek` app dir outside the user's home AND the document
...
/// checkout (has a `.git` entry). An explicit `$GHOSTY_HOME` config is
/// user-global wherever that home points, even when the directory itself
/// happens to be named `.ghosty`; other custom locations (for example
/// `GHOSTY_CONFIG_PATH=~/team.toml` or an isolated test directory) stay
...
pub fn config_path_is_workspace_scoped(path: &Path) -> bool {
    config_path_is_workspace_scoped_with_context(
        ...
        ghosty_paths::ghosty_home_override()
            ...
            .flatten()
            .as_deref(),
        ghosty_paths::user_home().as_deref(),
...
fn config_path_is_workspace_scoped_with_context(
    path: &Path,
    explicit_ghosty_home: Option<&Path>,
    user_home: Option<&Path>,
    current_dir: Option<&Path>,
) -> bool {
    if let Some(home) = explicit_ghosty_home
    ...
    let parent_is_app_dir = parent
        ...
        .is_some_and(|name| name == GHOSTY_APP_DIR || name == LEGACY_APP_DIR);
...
mod credential_scope_tests {
    ...
    fn config_inside_current_workspace_is_workspace_scoped() {
        ...
        for app_dir in [".ghosty", ".deepseek"] {
    ...
    fn checkout_config_outside_cwd_is_workspace_scoped_via_git_marker() {
        ...
        std::fs::create_dir_all(repo.join(".ghosty")).expect("app dir");
        assert!(config_path_is_workspace_scoped_with_context(
            &repo.join(".ghosty/config.toml"),
            None,
            Some(Path::new("/home/user")),
            Some(Path::new("/somewhere/else")),
        ));
    ...
    fn user_global_and_custom_locations_are_not_workspace_scoped() {
        ...
        let temp = tempfile::tempdir().expect("tempdir");
        assert!(!config_path_is_workspace_scoped_with_context(
            &temp.path().join(".ghosty/config.toml"),
            None,
            Some(home),
            elsewhere,
        ));
    ...
    #[test]
    fn explicit_ghosty_home_config_is_user_global_even_when_dir_is_app_named() {
        let temp = tempfile::tempdir().expect("tempdir");
        let repo = temp.path().join("repo");
        let explicit = repo.join(".ghosty");
        // Even with cwd inside the repo, the explicit GHOSTY_HOME config is
        ...
        assert!(config_path_is_workspace_scoped_with_context(
            &repo.join("other/.ghosty/config.toml"),
            Some(&explicit),
            Some(Path::new("/home/user")),
...
fn permission_removal_token(path: &Path, raw: &str, index: usize) -> String {
    ...
    hasher.update(b"ghosty-permission-removal-v1\0");
...
pub fn default_config_path() -> Result<PathBuf> {
    // Prefer ~/.ghosty/config.toml when it exists (fresh install or
    // migrated), otherwise fall back to ~/.deepseek/config.toml.
    let primary = ghosty_home()?.join(CONFIG_FILE_NAME);
    if ghosty_home_is_explicit() || primary.exists() {
        return Ok(primary);
    }
...
impl ConfigMigration {
    pub fn user_notice(&self) -> String {
        format!(
            "Migrated legacy config from {} to {}. Use the .ghosty path for future edits; the .deepseek file remains only as a compatibility fallback.",
            self.legacy_path.display(),
            self.primary_path.display()
        )
    }
...
/// `~/.ghosty/config.toml`. Called on first launch after the config
...
pub fn migrate_config_if_needed() -> Result<Option<ConfigMigration>> {
    ...
    if let Some(parent) = primary.parent() {
        std::fs::create_dir_all(parent).context("failed to create ghosty config directory")?;
    }
    std::fs::copy(&legacy, &primary)
        .context("failed to migrate config from deepseek to ghosty home")?;
    tracing::info!(
...
struct EnvRuntimeOverrides {
    ...
    /// `GHOSTY_TELEMETRY`/`DEEPSEEK_TELEMETRY` was set to something
    ...
    /// `GHOSTY_TELEMETRY_ENDPOINT`/`DEEPSEEK_TELEMETRY_ENDPOINT`. Overrides
...
impl EnvRuntimeOverrides {
    fn load() -> Self {
        ...
        Self {
            provider,
            provider_source,
            model: std::env::var("GHOSTY_MODEL")
            ...
            arcee_model: std::env::var("ARCEE_MODEL")
                .ok()
                .filter(|v| !v.trim().is_empty()),
            verbosity: std::env::var("GHOSTY_VERBOSITY")
                .or_else(|_| std::env::var("DEEPSEEK_VERBOSITY"))
                .ok(),
            output_mode: std::env::var("GHOSTY_OUTPUT_MODE")
                .or_else(|_| std::env::var("DEEPSEEK_OUTPUT_MODE"))
                .ok(),
            auth_mode: std::env::var("GHOSTY_AUTH_MODE")
                .or_else(|_| std::env::var("DEEPSEEK_AUTH_MODE"))
                .ok(),
            log_level: std::env::var("GHOSTY_LOG_LEVEL")
            ...
            // `GHOSTY_TELEMETRY_ENDPOINT=` select the shipped endpoint —
            // the opposite of what anyone typing it means. Resolution reads an
            // empty override as "contact nobody, write the dry-run file".
            telemetry_endpoint: std::env::var("GHOSTY_TELEMETRY_ENDPOINT")
                .or_else(|_| std::env::var("DEEPSEEK_TELEMETRY_ENDPOINT"))
                .ok(),
            approval_policy: std::env::var("GHOSTY_APPROVAL_POLICY")
                .or_else(|_| std::env::var("DEEPSEEK_APPROVAL_POLICY"))
                .ok(),
            sandbox_mode: std::env::var("GHOSTY_SANDBOX_MODE")
                .or_else(|_| std::env::var("DEEPSEEK_SANDBOX_MODE"))
                .ok(),
            yolo: std::env::var("GHOSTY_YOLO")
                .or_else(|_| std::env::var("DEEPSEEK_YOLO"))
                .ok()
                .and_then(|v| match parse_bool(&v) {
                    Ok(b) => Some(b),
                    Err(_) => {
                        tracing::warn!("Invalid GHOSTY_YOLO/DEEPSEEK_YOLO value '{v}', expected true/false");
                        None
                    }
                }),
            http_headers: std::env::var("GHOSTY_HTTP_HEADERS")
                .or_else(|_| std::env::var("DEEPSEEK_HTTP_HEADERS"))
                .ok()
                .and_then(|value| match parse_http_headers(&value) {
                    Ok(h) => Some(h),
                    Err(_) => {
                        tracing::warn!("Invalid GHOSTY_HTTP_HEADERS/DEEPSEEK_HTTP_HEADERS value, expected format: header1=val1,header2=val2");
                        None
                    }
                })
                .filter(|headers| !headers.is_empty()),
            deepseek_base_url: std::env::var("GHOSTY_BASE_URL")
    ...
    fn load_provider() -> (Option<ProviderKind>, Option<&'static str>) {
        if let Ok(value) = std::env::var("GHOSTY_PROVIDER") {
            let parsed = ProviderKind::parse_config_identity(&value);
            return (parsed, parsed.map(|_| "GHOSTY_PROVIDER"));
        }
    ...
    fn base_url_for(&self, provider: ProviderKind) -> Option<String> {
        ...
        match provider {
            ...
            // No dedicated GHOSTY_CUSTOM_BASE_URL env override: a custom
