...
impl ConfigToml {
    ...
    pub fn resolve_runtime_options_with_secrets(
        ...
        let (api_key, api_key_source) = if let Some(value) = cli.api_key.clone() {
            ...
            match ghosty_secrets::env_for(provider.as_str()) {
...
// ── Ghosty Code state root (v0.8.44) ──────────────────────────────────
//
...
/// Canonical Ghosty Code app directory name under $HOME.
...
/// Resolve the primary Ghosty Code home directory.
///
...
pub fn ghosty_home() -> Result<PathBuf> {
    if let Ok(val) = std::env::var("GHOSTY_HOME") {
...
/// Resolve a state subdirectory, preferring the Ghosty Code root if
...
/// Ensure a state subdirectory exists under the primary Ghosty Code root,
...
pub fn resolve_config_path(explicit: Option<PathBuf>) -> Result<PathBuf> {
    let path = if let Some(path) = explicit {
        path
    } else if let Ok(path) = std::env::var("GHOSTY_CONFIG_PATH") {
...
impl EnvRuntimeOverrides {
    fn load() -> Self {
        Self {
            provider: std::env::var("GHOSTY_PROVIDER")
...
mod tests {
    ...
    impl ghosty_secrets::KeyringStore for RecordingSecretsStore {
        ...
        fn set(&self, _key: &str, _value: &str) -> Result<(), ghosty_secrets::SecretsError> {
            Ok(())
        }

        fn delete(&self, _key: &str) -> Result<(), ghosty_secrets::SecretsError> {
            Ok(())
        }
...
"#;
    ...
    fn app_homes_prefer_home_env_before_platform_home_fallback() {
        ...
        let home =
            std::env::temp_dir().join(format!("ghosty-config-home-env-{}", std::process::id()));
        let userprofile =
    ...
    fn migrate_config_reports_copied_legacy_path() {
        ...
        unsafe {
            ...
            env::set_var("GHOSTY_HOME", &primary_dir);
        }
