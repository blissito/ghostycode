...
/// Persist `tui.status_items` to `~/.ghosty/config.toml` without disturbing
...
/// Resolve the path to `~/.ghosty/config.toml` (or
/// `$GHOSTY_CONFIG_PATH` / `$DEEPSEEK_CONFIG_PATH`). Mirrors what `Config::load` accepts so we
...
pub(super) fn config_toml_path(config_path: Option<&Path>) -> anyhow::Result<PathBuf> {
    ...
    if let Ok(env) = std::env::var("GHOSTY_CONFIG_PATH") {
...
You are the ghosty auto-routing classifier. Return only compact JSON: \
...
/// `ghosty auth clear --provider <id>` and
...
mod tests {
    ...
    impl Drop for EnvGuard {
        fn drop(&mut self) {
            ...
            if let Some(value) = self.ghosty_config_path.take() {
                // Safety: test-only environment mutation guarded by a global mutex.
                unsafe {
                    env::set_var("GHOSTY_CONFIG_PATH", value);
                }
            } else {
                // Safety: test-only environment mutation guarded by a global mutex.
                unsafe {
    ...
    fn config_command_allow_shell_save_persists_root_boolean() {
        let temp_root = env::temp_dir().join(format!(
            "ghosty-allow-shell-save-app-path-test-{}",
            std::process::id()
        ));
        fs::create_dir_all(&temp_root).unwrap();
