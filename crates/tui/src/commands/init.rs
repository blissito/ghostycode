...
/// If `workspace` is inside a git repository, ensure workspace-local Ghosty Code
...
/// keeping the authored `.ghosty/constitution.json` repo authority policy
/// committable (a directory exclude cannot be overridden, so `.ghosty/*` plus
...
fn generate_project_doc(workspace: &Path) -> String {
    ...
    doc.push('\n');
    doc.push_str("- **Ghosty Code reads this file as:** AGENTS.md (canonical cross-agent project instructions). <!-- WHALE.md is deprecated; put Ghosty Code-specific authority policy in .ghosty/constitution.json -->\n");
    doc.push_str(
...
"#;
    ...
    fn ensure_deepseek_gitignored_creates_gitignore() {
        ...
        // .ghosty/ is ignored at any depth, but the committed
        // constitution.json is kept.
        assert!(content.contains("**/.ghosty/*"));
        assert!(content.contains("!**/.ghosty/constitution.json"));
    }
