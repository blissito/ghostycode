...
/// Return true when `GHOSTY_LOG_LEVEL` (legacy alias: `DEEPSEEK_LOG_LEVEL`)
...
mod tests {
    ...
    fn log_value_parser_accepts_common_rust_log_directives() {
        ...
        assert!(log_value_enables_verbose("ghosty_cli=debug"));
        assert!(log_value_enables_verbose("warn,ghosty_tui::client=trace"));
        assert!(!log_value_enables_verbose("warn"));
        assert!(!log_value_enables_verbose("ghosty_tui=off"));
    }
