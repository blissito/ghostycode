//! CLI entry point for Ghosty Code.
...
#[command(
    ...
    about = "Ghosty Code terminal coding agent",
...
enum Commands {
    ...
    Exec(ExecArgs),
    /// Generate SWE-bench prediction rows from Ghosty Code runs
...
enum SwebenchCommand {
    /// Run Ghosty Code on one SWE-bench instance and export the resulting diff
    Run(SwebenchRunArgs),
...
enum McpCommand {
    ...
    Validate,
    /// Register this Ghosty Code binary as a local MCP stdio server.
    ///
    ...
    #[command(
        name = "add-self",
        long_about = "Register this Ghosty Code binary as a local MCP stdio server.\n\nAdds a config entry to ~/.ghosty/mcp.json that launches `ghosty serve --mcp`\nvia the stdio transport. Other Ghosty Code sessions (or any MCP client) can then\ndiscover and call tools exposed by this server.\n\nUse `ghosty serve --http` instead if you need the HTTP/SSE runtime API."
    )]
    AddSelf {
    ...
    match args.command {
        SwebenchCommand::Run(args) => {
            ...
            let model_name = args
                .model_name_or_path
                .clone()
                .unwrap_or_else(|| format!("ghosty/{model}"));
...
fn swebench_prompt(
    ...
    prompt.push_str("Edit the workspace to resolve the issue. Run targeted tests when practical. Do not commit, tag, publish, or change remotes. Leave the final solution as a working-tree diff; Ghosty Code will export that diff as the SWE-bench prediction.\n\n");
...
fn tools_readme_template() -> &'static str {
    "# Local tools\n\n\
     ...
     `ghosty-tui setup --status` and surfaced in `ghosty-tui doctor`.\n\n\
...
fn tools_example_script() -> &'static str {
    "#!/usr/bin/env sh\n\
     ...
     # usage: example [name]\n\
     printf 'ghosty-tui local tool ok: %s\\n' \"${1:-world}\"\n"
}
...
fn plugins_readme_template() -> &'static str {
    "# Local plugins\n\n\
     ...
     experiments without touching `~/.ghosty/skills/`.\n\n\
...
fn deepseek_home_dir() -> PathBuf {
    ghosty_config::ghosty_home().unwrap_or_else(|_| {
        dirs::home_dir().map_or_else(|| PathBuf::from(".ghosty"), |h| h.join(".ghosty"))
    })
}
        ...
        ApiKeySource::Missing => {
            let (env_var, login_hint) = match config.api_provider() {
                crate::config::ApiProvider::NvidiaNim => (
                    "NVIDIA_API_KEY",
                    "ghosty auth set --provider nvidia-nim --api-key \"...\"",
                ),
                crate::config::ApiProvider::Openai => (
                    "OPENAI_API_KEY",
                    "ghosty auth set --provider openai --api-key \"...\"",
                ),
                crate::config::ApiProvider::Atlascloud => (
                    "ATLASCLOUD_API_KEY",
                    "ghosty auth set --provider atlascloud --api-key \"...\"",
                ),
                crate::config::ApiProvider::WanjieArk => (
                    "WANJIE_ARK_API_KEY",
                    "ghosty auth set --provider wanjie-ark --api-key \"...\"",
                ),
                crate::config::ApiProvider::Openrouter => (
                    "OPENROUTER_API_KEY",
                    "ghosty auth set --provider openrouter --api-key \"...\"",
                ),
                ...
                crate::config::ApiProvider::Fireworks => (
                    "FIREWORKS_API_KEY",
                    "ghosty auth set --provider fireworks --api-key \"...\"",
                ),
                crate::config::ApiProvider::Siliconflow
                | crate::config::ApiProvider::SiliconflowCn => (
                    "SILICONFLOW_API_KEY",
                    "ghosty auth set --provider siliconflow --api-key \"...\"",
                ),
                crate::config::ApiProvider::Arcee => (
                    "ARCEE_API_KEY",
                    "ghosty auth set --provider arcee --api-key \"...\"",
                ),
                crate::config::ApiProvider::Moonshot => (
                    "MOONSHOT_API_KEY/KIMI_API_KEY",
                    "ghosty auth set --provider moonshot --api-key \"...\"",
                ),
                crate::config::ApiProvider::Sglang => (
                    "SGLANG_API_KEY",
                    "ghosty auth set --provider sglang --api-key \"...\"",
                ),
                crate::config::ApiProvider::Vllm => (
                    "VLLM_API_KEY",
                    "ghosty auth set --provider vllm --api-key \"...\"",
                ),
                crate::config::ApiProvider::Ollama => {
                    ("OLLAMA_API_KEY", "ghosty auth set --provider ollama")
                }
                crate::config::ApiProvider::Volcengine => (
                    "VOLCENGINE_API_KEY",
                    "ghosty auth set --provider volcengine",
                ),
                crate::config::ApiProvider::Huggingface => (
                    "HUGGINGFACE_API_KEY/HF_TOKEN",
                    "ghosty auth set --provider huggingface",
                ),
                crate::config::ApiProvider::Deepseek | crate::config::ApiProvider::DeepseekCN => {
                    ("DEEPSEEK_API_KEY", "ghosty auth set --provider deepseek")
                }
            };
            println!(
                "  {} api_key: missing  (set {env_var} or `[providers.{}].api_key` in ~/.ghosty/config.toml; or run `{login_hint}`)",
...
async fn run_doctor(config: &Config, workspace: &Path, config_path_override: Option<&Path>) {
    ...
    println!(
        "{}",
        "ghosty Doctor".truecolor(blue_r, blue_g, blue_b).bold()
    );
    println!("{}", "==================".truecolor(sky_r, sky_g, sky_b));
    println!();
    ...
    println!("  ghosty-tui: {}", env!("DEEPSEEK_BUILD_VERSION"));
    ...
    match ghosty_release::latest_release_tag_async(ghosty_release::ReleaseChannel::Stable).await {
        Ok(latest_tag) => {
            match ghosty_release::compare_release_versions(current_version, &latest_tag) {
                Ok(std::cmp::Ordering::Less) => {
                    println!(
                        "  {} latest: {latest_tag}",
                        "!".truecolor(sky_r, sky_g, sky_b)
                    );
                    println!("    Update available. Run `ghosty update` to install.");
                }
        ...
        Err(err) => {
            ...
            println!("    Run `ghosty update --check` to retry.");
        }
    }
    println!();
    ...
    let config_path = config_path_override
        .map(PathBuf::from)
        .or_else(|| ghosty_config::resolve_config_path(None).ok())
        .unwrap_or_else(|| {
            ghosty_config::ghosty_home()
                .unwrap_or_else(|_| PathBuf::from(".ghosty"))
                .join("config.toml")
        });
    ...
    if active_root != &code_home {
        println!(
            "  note: legacy {} found; migrate with `ghosty setup --migrate`",
    ...
    println!("  · credential precedence: ~/.ghosty/config.toml, OS keyring, then env");
    ...
    let has_api_key = if config.deepseek_api_key().is_ok() {
        ...
        println!(
            "    Run 'ghosty auth set --provider <name>' to save a key to ~/.ghosty/config.toml."
        );
        false
    };
            ...
            Err(e) => {
                ...
                if error_msg.contains("401") || error_msg.contains("Unauthorized") {
                    ...
                    if matches!(api_key_source, ApiKeySource::Keyring) {
                        ...
                        println!(
                            "    Run `ghosty auth status` to inspect config/keyring/env sources."
                        );
                        ...
                        println!(
                            "    Run `ghosty auth set --provider deepseek` to save a config key that overrides stale env."
                        );
                    }
    ...
    if mcp_config_path.exists() {
        ...
        println!("    Run `ghosty mcp init` or `ghosty setup --mcp`.");
    }
    ...
    let stash_path = ghosty_config::ghosty_home()
        .ok()
...
fn run_doctor_json(
    ...
    let report = json!({
        ...
        "storage": {
            ...
            "stash": {
                "path": ghosty_config::ghosty_home()
                    .ok()
                    .map(|h| h.join("composer_stash.jsonl").display().to_string())
                    .unwrap_or_default(),
                "present": ghosty_config::ghosty_home()
                    .ok()
                    .map(|h| h.join("composer_stash.jsonl"))
        ...
        "api_connectivity": {
            "checked": false,
            "note": "Skipped in --json mode; run `ghosty doctor` for a live check.",
        },
...
fn list_sessions(limit: usize, search: Option<String>) -> Result<()> {
    ...
    if sessions.is_empty() {
        ...
        println!(
            ...
            "ghosty".truecolor(blue_r, blue_g, blue_b)
        );
        return Ok(());
    }
    ...
    println!(
        "Resume with: {} {}",
        "ghosty --resume".truecolor(blue_r, blue_g, blue_b),
        "<session-id>".dimmed()
    );
    println!(
        "Continue latest in this workspace: {}",
        "ghosty --continue".truecolor(blue_r, blue_g, blue_b)
    );
...
/// `ghosty sessions` / `ghosty resume <id>`) and cleared, but not loaded.
fn recover_interrupted_checkpoint_for_resume(launch_workspace: &Path) -> Option<String> {
    ...
    if !workspace_matches {
        // Persist the checkpoint so the user can find it via `ghosty
...
fn merge_project_config(config: &mut Config, workspace: &Path) {
    ...
    if let Some(v) = table.get("approval_policy").and_then(toml::Value::as_str)
        && !v.is_empty()
    {
        if ghosty_config::project_approval_policy_is_allowed(config.approval_policy.as_deref(), v) {
...
mod terminal_mode_tests {
    ...
    fn swebench_diff_export_excludes_runtime_artifacts() {
        ...
        std::process::Command::new("git")
            .arg("-C")
            .arg(repo)
            .args(["config", "user.name", "Ghosty Code"])
            .status()
            .expect("git config user.name");
        std::process::Command::new("git")
            .arg("-C")
            .arg(repo)
            .args(["config", "user.email", "ghosty@example.invalid"])
            .status()
        ...
        std::fs::create_dir_all(repo.join(".ghosty")).expect("mkdir .ghosty");
        std::fs::write(repo.join(".ghosty/instructions.md"), "generated")
    ...
    fn no_alt_screen_flag_is_accepted_but_keeps_alternate_screen() {
        let cli = parse_cli(&["ghosty", "--no-alt-screen"]);
        let config = Config::default();
...
mod doctor_mcp_tests {
    ...
    fn test_self_hosted_absolute_is_ok() {
        let server = make_server(Some("/usr/local/bin/ghosty"), &["serve", "--mcp"], None);
        match doctor_check_mcp_server(&server) {
    ...
    fn test_self_hosted_relative_is_warning() {
        let server = make_server(Some("ghosty"), &["serve", "--mcp"], None);
        match doctor_check_mcp_server(&server) {
            McpServerDoctorStatus::Warning(detail) => {
