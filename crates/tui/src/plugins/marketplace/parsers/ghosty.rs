//! Ghosty native catalog parser.
//!
//! This is Ghosty's own documented catalog format. It is deliberately
...
pub fn parse_ghosty_catalog(document: MarketplaceDocument) -> MarketplaceCatalog {
    ...
    let Some(obj) = root.as_object() else {
        return MarketplaceCatalog {
            id: catalog_id,
            format: MarketplaceFormat::Ghosty,
            name: String::new(),
            display_name: None,
            description: None,
            version: None,
            ...
            diagnostics: vec![MarketplaceDiagnostic::error(
                "NOT_AN_OBJECT",
                "Ghosty catalog must be a JSON object",
                None,
                None,
            )],
        };
    ...
    let Some(entries) = obj.get("plugins").and_then(Value::as_array) else {
        diagnostics.push(MarketplaceDiagnostic::error(
            "MISSING_PLUGINS",
            "Ghosty catalog must contain a `plugins` array",
            None,
            None,
        ));
        return MarketplaceCatalog {
    ...
    for (index, entry) in entries.iter().enumerate() {
        if let Some(candidate) = parse_ghosty_entry(&catalog_id, index, entry, &mut diagnostics) {
...
fn parse_ghosty_entry(
    ...
    let Some(obj) = entry.as_object() else {
        diagnostics.push(MarketplaceDiagnostic::error(
            "MALFORMED_ENTRY",
            format!("Ghosty plugin at index {index} must be a JSON object"),
            None,
            Some(index),
        ));
        return None;
    ...
    let Some(name) = name else {
        diagnostics.push(MarketplaceDiagnostic::error(
            "MISSING_NAME",
            format!("Ghosty plugin at index {index} is missing required `name`"),
            None,
            Some(index),
        ));
        return None;
    ...
    let Some(source) = source else {
        diagnostics.push(MarketplaceDiagnostic::error(
            "MISSING_SOURCE",
            format!("Ghosty plugin `{name}` is missing required `source`"),
            Some(name.clone()),
            Some(index),
        ));
        return None;
    ...
    let install_plan = match PluginInstallSource::parse(source) {
        ...
        Err(err) => MarketplaceInstallPlan::Unsupported {
            reason: format!("invalid Ghosty install spec: {err}"),
