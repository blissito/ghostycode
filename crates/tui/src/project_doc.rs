...
//! Priority: AGENTS.md > WHALE.md (deprecated) > .claude/instructions.md > CLAUDE.md > .ghosty/instructions.md > .deepseek/instructions.md

use std::path::{Path, PathBuf};

/// Document filenames to search for (in priority order).
/// `AGENTS.md` is canonical. `WHALE.md` is **deprecated** (read-only legacy
/// fallback, now below `AGENTS.md`); Ghosty Code-specific authority policy lives
/// in `.ghosty/constitution.json`. `CLAUDE.md` and the `*/instructions.md`
/// variants are read-only compatibility fallbacks.
pub const DOC_FILENAMES: &[&str] = &[
    "AGENTS.md",
    "WHALE.md", // deprecated: legacy Ghosty Code-native, read-only fallback
