...
pub struct Settings {
    ...
    /// - `"ghost"` (default, alias `"whale"`): the `👻 → 🫥` 12-frame
    ///   sequence — Ghosty builds dots then fades out at the midpoint.
    ///   Idle frame is a steady `👻`.
    /// - `"dots"`: the 6-frame geometric sequence (`◍ ◉ ◌ ◌ ◉ ◍`).
...
impl Default for Settings {
    fn default() -> Self {
        Self {
            ...
            status_indicator: "ghost".to_string(),
...
impl Settings {
    ...
    pub fn set(&mut self, key: &str, value: &str) -> Result<()> {
        match key {
            ...
            "status_indicator" | "indicator" => {
                ...
                if !["ghost", "dots", "off"].contains(&normalized) {
                    anyhow::bail!(
                        "Failed to update setting: invalid status indicator '{value}'. Expected: ghost, dots, off."
                    );
                }
    ...
    pub fn available_settings() -> Vec<(&'static str, &'static str)> {
        vec![
            ...
            (
                "status_indicator",
                "Header status indicator next to effort chip: ghost, dots, off",
            ),
            (
                "synchronized_output",
