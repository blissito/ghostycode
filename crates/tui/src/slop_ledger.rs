...
//! ## Design
//!
//! - **Storage**: `~/.ghosty/slop_ledger.json` (a JSON array of entries).
...
impl SlopLedger {
    ...
    pub fn default_path() -> io::Result<PathBuf> {
        ghosty_config::resolve_state_dir("slop_ledger")
...
/// to the portable `~/.ghosty/secrets` form.
fn redact_exported_text(text: &mut String) {
    ...
    if let Some(home) = dirs::home_dir() {
        for leaf in [".ghosty/secrets", ".deepseek/secrets"] {
            let dir = home.join(leaf);
            let prefix = dir.to_string_lossy().to_string();
            result = result.replace(&prefix, "~/.ghosty/secrets");
        }
    }
    *text = result;
}
