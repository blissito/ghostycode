...
//! (`~/.ghosty/snapshots` or legacy `~/.deepseek/snapshots`) with
...
/// `$STATE_DIR` is resolved via `ghosty_config::resolve_state_dir`.
...
fn snapshot_base_with_home(home: Option<PathBuf>) -> PathBuf {
    ...
    let primary = home.join(".ghosty").join("snapshots");
    if primary.exists() {
        return primary;
    }
