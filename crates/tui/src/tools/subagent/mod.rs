...
/// already-rendered `<ghosty:subagent.done>` sentinel that the model
...
fn default_state_path(workspace: &Path) -> PathBuf {
    ...
    let primary = workspace.join(".ghosty").join("state");
    if primary.exists() {
...
async fn run_subagent_task(task: SubAgentTask) {
    ...
    // (`ghosty:subagent.done`) to avoid collision with normal user
    // text.
...
/// Build a `<ghosty:subagent.done>` sentinel for a failed child.
///
...
You are the ghosty sub-agent routing manager. Return only compact JSON: \
