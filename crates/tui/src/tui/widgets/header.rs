...
pub fn header_status_indicator_frame(
    ...
    let frames: &[&str] = match mode.trim().to_ascii_lowercase().as_str() {
        ...
        // an emoji chip beside the model/mode cluster. The setting keeps its
        // `"cw"` value for compatibility; what it paints is the Ghosty mark.
        _ => return Some("ghosty"),
    };
