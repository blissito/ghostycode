...
# cargo test -p ghosty-tui --bin ghosty-tui --features long-running-tests commands::groups::session::acceptance -- --test-threads=1
Feature: Session command workflows
  ...
  Scenario: Save and export preserve data while load defers restoration
    ...
    And GhostyCode should defer the session-loaded receipt to the event loop
  ...
  Scenario: Context management commands emit actions without clearing the active session
    ...
    Then GhostyCode should send a session relay instruction focused on "handoff details"
    And the active session should contain the saved message
