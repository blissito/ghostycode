...
Measured facts about how long Ghosty takes to build and test, what was
...
| Step | Wall | Notes |
| --- | --- | --- |
| Cold `cargo build -p ghosty-tui` (empty target) | 94 s (user 270 s) | 543 units; `ghosty-tui` alone is 70 s and is the critical path; next longest units are `ghosty-config` 7.5 s, `jsonschema` 6.3 s, `ghosty-workflow` 5.6 s, `tokio` 5.4 s. Load 13. |
| Cold `cargo test -p ghosty-tui --lib --no-run` (empty target) | 148 s (user 347 s) | The 10.5k-test unit binary is 357 MB and trips the macOS linker's `__eh_frame > 16MB` compact-unwind warning (harmless). Load 20. |
| Incremental `cargo build -p ghosty-tui` after a one-line edit | 12.5 s | Load 11. |
| Incremental `cargo test -p ghosty-tui --lib --no-run` after a one-line edit | 19 s | Load 11. |
...
| Running the tui unit suite with libtest (`cargo test -p ghosty-tui --lib`) | 268 s | From the release gate log; 10,531 tests. Load ~10. |
| Running the same suite with `cargo nextest run -p ghosty-tui --lib` | 96–108 s | Same tests, one process per test, all cores busy. Load 12–20. |
...
| --- | --- | --- |
| Cold `cargo test -p ghosty-tui --lib --locked --no-run --timings` | 127 s (user 329 s) | 8.9 |
| `touch crates/tui/src/elapsed.rs` + same command | 21 s | 12.3 |
| `touch` + `cargo test -p ghosty-tui --lib --locked elapsed::` (the everyday loop) | 20 s (4 tests run) | 11.2 |
| Lib-test binary size | 357 MB (`ghosty_tui-<hash>`); links with the `__eh_frame section too large (max 16MB)` compact-unwind warning | — |
| `cargo check -p ghosty-tui --lib --tests` incremental after `touch` (frontend only) | 14 s | 6.2 |
| Incremental full lib-test build after `touch`, same conditions | 28 s | 6.2 |

Cold timing report, top units (605 units): `ghosty-tui` lib test
**106.0 s**, `ghosty-config` 7.9 s, `jsonschema` 5.8 s, `moxcms` 4.7 s,
`ghosty-protocol` 4.2 s, `tokio` 4.0 s, `rustls` 3.8 s, `schemaui`
3.4 s, `rmcp` 3.4 s, `h2` 3.3 s, `jsonschema` (second copy) 3.2 s,
`ghosty-workflow` 3.2 s, `syn` 3.1 s, `rio-vt` 3.1 s, `regex-automata`
3.0 s. The incremental report has exactly one non-zero unit: `ghosty-tui`
...
`cargo llvm-lines -p ghosty-tui --lib`: **8,138,810 lines in 223,052
...
ghosty-workflow-js 61 tests and the tui schema tests pass). Cold saving
is the two duplicated units (~9 s of unit time, ~3 s of wall).

### A1 cache topology (desk-local, not committed)

New-worktree cold `cargo test -p ghosty-tui --lib --locked --no-run`,
...
build-dir = "/path/to/cache/ghosty/build/{workspace-path-hash}"
...
| --- | --- | --- |
| **Compile-time** | `scripts/dev-test.sh` / `scripts/dev-cargo.sh` set `CARGO_BUILD_BUILD_DIR=$GHOSTY_CACHE_ROOT/build/{workspace-path-hash}`, so concurrent worktrees do not share a Cargo lock. A leftover `./target` (Cargo still writes `CACHEDIR.TAG` there when build-dir is split) does **not** turn isolation off; `GHOSTY_DEV_CACHE=local` keeps `./target` if you want that. Cargo older than 1.91 falls back to a per-workspace `CARGO_TARGET_DIR`. | Not a smaller rustc unit. Workspace crates still rebuild. |
| **Compile-time (sccache)** | `RUSTC_WRAPPER=sccache` and `SCCACHE_DIR=$GHOSTY_CACHE_ROOT/sccache/<rustc-commit>` only when incremental is already off (`CARGO_INCREMENTAL=0` or `GHOSTY_SCCACHE=1`) **and** `sccache` is on `PATH`. | Not enabled on the everyday incremental loop. sccache cannot cache incremental units; wrapping those builds adds overhead and 0% hits. Missing sccache is a printed fallback, not an error. |
| **Test-runtime** | `scripts/dev-test.sh` uses `cargo nextest run` when `cargo-nextest` is installed (`GHOSTY_DEV_NEXTEST=0` forces libtest). Same binaries; process per test. Retries stay 0. `RUST_MIN_STACK=16MiB` is exported when unset. | Not a compile win. nextest does not run doctests; `cargo test --doc` remains a separate gate. |
...
```sh
# Portable default:
#   ${XDG_CACHE_HOME:-$HOME/.cache}/ghosty
# Desk override, if you want the cache on a particular volume:
export GHOSTY_CACHE_ROOT=/path/to/cache/ghosty

scripts/dev-cache.sh --self-check
scripts/dev-test.sh crates/tui/src/elapsed.rs
CARGO_INCREMENTAL=0 scripts/dev-cargo.sh test -p ghosty-config --lib --locked --no-run
```
...
1.97.0, cargo 1.97.0, sccache 0.17.0. `GHOSTY_CACHE_ROOT` set to a
...
this worktree. The first isolated `ghosty-config` `--no-run` created a
...
helper now stays isolated unless `GHOSTY_DEV_CACHE=local` or `0`.

**Compile-time** (`scripts/dev-cargo.sh test … --locked --offline --no-run`):

| Step | Wall | Notes |
| --- | ---: | --- |
| First isolated `ghosty-config --lib --no-run` | 9.14 s (user 23.8 s) | 90 units into the hashed build-dir |
| Warm isolated same command (after the stub-target fix) | 0.13 s | `Finished` in 0.07 s |
| `touch crates/config/src/lib.rs` + isolated `--no-run` | 0.93 s | only `ghosty-config` rebuilt |
| First isolated `ghosty-tui --lib --no-run` | **121.5 s** (user 305 s) | 600 units; 340 MB binary; A0 empty-target was 127 s / 329 s |
| `touch crates/tui/src/elapsed.rs` + isolated `--no-run` | **18.15 s** | everyday compile loop; A0 was 21 s / 19 s |
| `GHOSTY_SCCACHE=1` config `--no-run` on the already-warm tree | 5.21 s then 0.14 s | wrapper and `SCCACHE_DIR=…/sccache/<rustc-commit>` set; 0 sccache hits because only workspace crates recompiled and the build-dir was not emptied |
...
| `GHOSTY_DEV_NEXTEST=0 scripts/dev-test.sh config` (libtest) | body 0.11 s / real 0.27 s | 557 tiny tests: process-per-test is slower here |
...
| --- | --- | --- | --- | --- |
| `ghosty-tui` lib (dev) | incremental, cgu 256 | 3.3 GB | 12–14 s | 6.3 |
| `ghosty-tui` lib test | incremental, cgu 256 | 6.0 GB | 21–28 s | 6.3 |
| `ghosty-tui` lib (dev) | non-incremental (`CARGO_INCREMENTAL=0`), cgu 16 | **6.0 GB** | 78 s | 5.5 |
| `ghosty-tui` lib test | non-incremental, cgu 16 | **8.0 GB** | 105 s | 5.5 |
| `ghosty-tui` lib test | non-incremental, `codegen-units = 4` | 6.1 GB (−24 %) | 145 s (+38 %) | 5.5 |
| `ghosty-tui` lib test | non-incremental, `codegen-units = 1` | 7.8 GB (−3 %) | 161 s (+53 %) | 5.5 |
| next-largest units (ghosty-config, rmcp, tokio, schemaui, ghosty-workflow) | either | 0.4–0.7 GB | — | — |

So a plain `cargo build -p ghosty-tui` needs ~6 GB for one rustc, the
...
cargo build -p ghosty-tui
cargo test  -p ghosty-tui --lib -- <filter>
...
ghosty-tui library exposes four `pub` items in total. Every one of them
...
### B2 landed (leaf types out of ghosty-tui)

| Move | Lines out of tui | Consumers changed |
| --- | --- | --- |
| `core/tool_parser.rs` → `ghosty_core::tool_parser` | 662 | 0 (re-export; integration harness imports instead of `#[path]`) |
| `tls.rs` → `ghosty_release::tls` | 21 | 0 (`use ghosty_release::tls;` at the crate root) |
| `AppMode` (+ pure impl) → `ghosty_config::AppMode`; localized picker strings stay as `AppModeUi` | ~150 | 3 files import the trait |
| `ApprovalMode` (+ pure impl) → `ghosty_execpolicy::ApprovalMode` | ~60 | 0 (re-export) |
...
1. `ApiProvider` + the exact-route helpers (`is_exact_*_route`) out of
   `crates/tui/src/config.rs` into ghosty-config, unblocking
   `ReasoningEffort`.
2. `localization` + `locales/*.json` → `ghosty-i18n` (the 312 k-line
   `rust_i18n` closure leaves the tui unit; locale-only edits stop
   rebuilding the TUI).
3. `palette` + `glyphs` → `ghosty-palette` (also fixes web/CWC token
   drift).
4. `client/` (provider wire adapters) → `ghosty-client`, then
...
2. **`cargo nextest` is supported and documented** (`.config/nextest.toml`).
   ...
   `ghosty` executable is capped at four concurrent tests so its 30 s
...
3. **Three tests depended on test order** and only passed because another
   ...
   `ghosty-tui mcp::sse::endpoint_tests::message_before_endpoint_is_rejected_instead_of_buffered`,
   `ghosty-app-server tests::failed_config_set_keeps_the_stdio_bridge`,
...
- `[profile.dev.package."*"] opt-level = 1` (dependencies optimized once,
  ...
  target layout: cold `cargo test -p ghosty-tui --lib --no-run` from an
  ...
  people trying to build Ghosty for the first time. Contributors who
...
order (each already only depends on `ghosty-config`/`ghosty-paths`
...
| --- | --- | --- | --- |
| `ghosty-glyphs` | `crates/tui/src/tui/glyphs.rs` | Constant tables + pure fns; no crate-internal deps. | `crate::tui::glyphs` |
| `ghosty-palette` | `crates/tui/src/palette/{tokens,themes,adapt,contrast,detect,osc11,user_theme}.rs` + `assets/user-theme.schema.json` | Pure color math and theme tables; depends on ratatui `Color` and `ghosty_config::ghosty_home` only. Also unblocks the web/CWC token drift noted in the tokens survey. | `crate::palette` |
| `ghosty-i18n` | `crates/tui/src/localization.rs` + `crates/tui/locales/*.json` | The `rust_i18n::i18n!` macro compiles all 15 packs into whichever crate hosts it; moving it out means locale-only edits no longer rebuild the TUI. `MessageId` is a plain enum. | `crate::localization` |
| `ghosty-mcp-transport` | `crates/tui/src/mcp/{sse,stdio,external_import}.rs` | Already talks to `ghosty-mcp`; the reviewed-launch binding is the only tui coupling. | `crate::mcp` |
...
`cargo test -p ghosty-tui --lib --no-run`). Expected win: the tui
...
- `scripts/dev-cargo.sh` / `scripts/dev-test.sh` — isolated `build-dir`
  ...
  `GHOSTY_CACHE_ROOT`; do not commit a machine path.
