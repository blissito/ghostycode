# Ghosty Code Classroom / Lab Install Checklist

A step-by-step checklist for IT admins deploying Ghosty Code on lab or classroom
...
| 4 | Obtain the installer: download `Ghosty CodeSetup.exe` from a v0.8.50+ [release](https://github.com/blissito/ghostycode/releases/latest) or from your department mirror. | ☐ |
| 5 | Verify SHA-256 hash against `ghosty-artifacts-sha256.txt` before deploying. | ☐ |
...
- Installs to `%LOCALAPPDATA%\Programs\Ghosty Code\bin`
...
1. Double-click `Ghosty CodeSetup.exe`.
...
```powershell
# 1. Create directory
$binDir = "$env:LOCALAPPDATA\Programs\Ghosty Code\bin"
...
|---|---------|-----------------|-------|
| 1 | `ghosty --version` | Prints version string | ☐ |
| 2 | `ghosty doctor` | All checks pass | ☐ |
| 3 | `ghosty-tui --version` | Prints version string | ☐ |
...
previous Ghosty Code install:

| # | Scenario | Expected result | Done? |
|---|----------|-----------------|-------|
| 1 | Install with no existing Ghosty Code PATH entry | Adds exactly `%LOCALAPPDATA%\Programs\Ghosty Code\bin` | ☐ |
| 2 | Install twice | PATH is not duplicated | ☐ |
| 3 | Install with a neighboring PATH entry such as `C:\Tools\Ghosty Code\bin-extra` | Neighboring entry is preserved | ☐ |
| 4 | Upgrade by installing a newer `Ghosty CodeSetup.exe` over an older one | Apps & Features version and both `--version` outputs match the new build | ☐ |
...
```powershell
& "$env:LOCALAPPDATA\Programs\Ghosty Code\Uninstall.exe" /S
```
...
| Symptom | Fix |
|---------|-----|
| `ghosty` not found after install | Open a **new** terminal. If still missing, check PATH: `echo $env:Path` |
| `MISSING_COMPANION_BINARY` | Ensure both `ghosty.exe` and `ghosty-tui.exe` are in the same directory |
...
| `ghosty doctor` fails API check | Verify `OPENAI_API_KEY` is set or `config.toml` exists |
...
1. Install Ghosty Code using Option A (silent) or Option C (manual).
2. Do **not** set API keys in the image — these are per-user/per-student.
3. The install directory (`%LOCALAPPDATA%\Programs\Ghosty Code\bin`) is per-user,
...
4. Alternatively, install to a shared location like `C:\Tools\Ghosty Code\bin`
   and add it to the **machine** PATH:
   ```powershell
   [Environment]::SetEnvironmentVariable("Path", "$env:Path;C:\Tools\Ghosty Code\bin", "Machine")
   ```
...
|------|-----------------|
| Binaries | `%LOCALAPPDATA%\Programs\Ghosty Code\bin\` |
| User config | `%APPDATA%\ghosty\config.toml` |
| Uninstaller | `%LOCALAPPDATA%\Programs\Ghosty Code\Uninstall.exe` |
