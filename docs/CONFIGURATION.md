...
- **`.ghosty/constitution.json`** — Ghosty Code-specific **repo authority /
  prioritization policy**: when local sources conflict, which should Ghosty Code
  ...
  This is the **local-law** layer in Ghosty Code's hierarchy: *base myth & global
  Constitution* (the model prompt in `prompts/base.md`, including the Brother
  Whale identity anchor) → *repo constitution* (`.ghosty/constitution.json`,
...
> Ghosty Code still **reads** an existing `WHALE.md` (below `AGENTS.md`) so old
...
> Ghosty Code-specific authority policy to `.ghosty/constitution.json`. (The
> global Ghosty Code Constitution shipped in the model prompt is a separate thing
> and is unaffected.)
...
The `ghosty` facade and `ghosty-tui` binary share the same config file for
DeepSeek auth and model defaults. `ghosty auth set --provider deepseek` (and
the legacy `ghosty login --api-key ...` alias) saves the key to
`~/.ghosty/config.toml` (migrating legacy `~/.deepseek/config.toml` on first
launch when needed), and `ghosty --model deepseek-v4-flash` is forwarded to
the TUI as `DEEPSEEK_MODEL`.

Credential lookup uses `config -> keyring -> env` after any explicit CLI
`--api-key`. Run `ghosty auth status` to inspect the active provider's config
...
`ghosty auth set --provider arcee --api-key "YOUR_ARCEE_API_KEY"`
...
`http://localhost:11434/v1` and sends model tags such as `ghostycoder:1.3b`
...
and uses `trinity-large-thinking` by default for Ghosty Code agent work.
...
Ghosty Code's chat provider and `image_analyze` tool are configured separately.
...
To bootstrap MCP and skills directories at their resolved paths, run `ghosty-tui setup`.
To only scaffold MCP, run `ghosty-tui mcp init`.

Note: setup, doctor, mcp, features, sessions, resume/fork, exec, review, and eval
are subcommands of the `ghosty-tui` binary. The `ghosty` dispatcher exposes a
...
By default, the TUI starts a background check for the latest stable Ghosty Code
...
`tag_name` field is accepted; if `assets` are present, Ghosty Code requires the
...
default_text_model = "ghostycoder:1.3b"
```
...
- `GHOSTY_HOME` (override the base data directory; defaults to `~/.ghosty`).
  ...
  the old env var is not used for new Ghosty Code state paths.
...
- `permissions.toml` (sibling file, optional): ask-only typed permission rule
  ...
  `~/.ghosty/permissions.toml`. This schema foundation accepts
...
- `mcp_config_path` (string, optional): defaults to `~/.ghosty/mcp.json`, with
  legacy `~/.deepseek/mcp.json` fallback when the Ghosty Code path is absent.
...
- `notes_path` (string, optional): defaults to `~/.ghosty/notes.txt`, with
  legacy `~/.deepseek/notes.txt` fallback when the Ghosty Code path is absent, and
...
- `memory_path` (string, optional): defaults to `~/.ghosty/memory.md`, with
  legacy `~/.deepseek/memory.md` fallback when the Ghosty Code path is absent.
...
Ghosty Code loads a small core native tool catalog by default and leaves less
...
- `ghosty-tui --enable web_search`
- `ghosty-tui --disable subagents`

Use `ghosty-tui features list` to inspect known flags and their effective state.
...
## Notes On `ghosty-tui doctor`

`ghosty-tui doctor` follows the same config resolution rules as the rest of the
...
To bootstrap missing MCP/skills paths, run `ghosty-tui setup --all`. You can
also run `ghosty-tui setup --skills --local` to create a workspace-local
`./skills` dir.

`ghosty-tui doctor --json` prints a machine-readable report that skips the
...
`ghosty-tui setup` accepts a few flags beyond the existing `--mcp`,
