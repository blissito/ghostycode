...
| COMPATIBILITY | `.deepseek/` state and stored provider aliases | Read fallback for upgraded installations and stored provider identity; writes remain Ghosty-only. Details are below. |
...
| COMPATIBILITY | Managed runtime envelope schema 1 | Previously persisted/enrolled runner replay; current local Ghosty Agent Mail producer is schema 2 and both normalize to the managed envelope. |
...
Ghosty was renamed from DeepSeek-TUI. To avoid breaking existing installs, the runtime reads
state from the new `~/.ghosty/` location but **falls back** to the legacy `~/.deepseek/` location,
and always **writes** to `~/.ghosty/`. This doc audits each legacy reference and records a
...
| Symbol | Line | Purpose |
|---|---|---|
| `GHOSTY_APP_DIR` / `LEGACY_APP_DIR` | 5369 | re-exported from `crates/paths` (defined at `crates/paths/src/lib.rs` 13 / 16) |
| `ghosty_home()` | 5375 | `~/.ghosty` |
| `legacy_deepseek_home()` | 5392 | `~/.deepseek` (legacy) |
| `resolve_state_dir(subdir)` | 5434 | **read** path: `~/.ghosty/<subdir>`, falling back to `~/.deepseek/<subdir>` when only the legacy dir exists |
| `ensure_state_dir(subdir)` | 5458 | **write** path: always creates under `~/.ghosty/<subdir>` |

Migration contract: read-with-fallback, write-to-new. This preserves the v0.8.44 migration for
users who still have `~/.deepseek/` while steering all new writes to `~/.ghosty/`.
...
that actively migrates `~/.deepseek/` → `~/.ghosty/` on first run and a deprecation window.
...
`resolve_state_dir`/`ensure_state_dir` instead of joining `.deepseek`/`.ghosty` by hand — is a
