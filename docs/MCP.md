...
ghosty can load additional tools via MCP (Model Context Protocol). MCP servers are local processes that the TUI starts and communicates with over stdio.
...
Server mode note:
- `ghosty-tui serve --mcp` runs the MCP stdio server.
- `ghosty-tui serve --http` runs the runtime HTTP/SSE API (separate mode).
- The `ghosty` dispatcher exposes `ghosty mcp-server` as an equivalent stdio
...
`ghosty-tui setup --mcp` performs the same MCP bootstrap alongside skills setup.

Common management commands:

```bash
ghosty-tui mcp list
ghosty-tui mcp tools [server]
ghosty-tui mcp add <name> --command "<cmd>" --arg "<arg>"
ghosty-tui mcp add <name> --url "http://localhost:3000/mcp"
ghosty-tui mcp enable <name>
ghosty-tui mcp disable <name>
ghosty-tui mcp remove <name>
ghosty-tui mcp validate
```
...
- `~/.ghosty/mcp.json` (`~/.deepseek/mcp.json` is still read when the Ghosty Code file is absent)
...
`ghosty-tui mcp init` (and `ghosty-tui setup --mcp`) writes to this resolved path.
...
```bash
ghosty-tui mcp add-self
```

This resolves the current binary path, generates a config entry that runs `ghosty-tui serve --mcp`, and writes it to your MCP config file. The default server name is `ghosty`.
...
The `ghosty-tui` binary supports `serve --mcp` directly. The `ghosty`
dispatcher offers the equivalent `ghosty mcp-server` stdio entrypoint. Use
whichever is on your `PATH` (run `which ghosty` or `which ghosty-tui` to
...
- `mcp_deepseek_<tool>` (if the server is named `ghosty`)

For example, the `shell` tool becomes `mcp_deepseek_shell`.

### MCP Server vs HTTP/SSE API vs ACP

| | `ghosty-tui serve --mcp` | `ghosty-tui serve --http` | `ghosty-tui serve --acp` |
|---|---|---|---|
...
- Run `ghosty-tui doctor` to confirm the MCP config path it resolved and whether it exists.
- In the TUI, run `/mcp validate` to refresh the visible server/tool snapshot.
- If the MCP config is missing, run `ghosty-tui mcp init --force` to regenerate it.
