...
Model Lab is the planned open-model workbench for Ghosty Code. The north star is
simple: Ghosty Code should become the best terminal coding agent for open-source
...
- OpenRouter, Novita, Fireworks, NVIDIA NIM, AtlasCloud, Wanjie Ark, generic
  ...
  paths where their IDs appear in `/provider`, `ghosty --provider`, or
  `ghosty models`.
...
describe a model as available before Ghosty Code can actually route to it.
