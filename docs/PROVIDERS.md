...
Ghosty codebase. It is intentionally conservative: shipped entries are
...
- `crates/agent/src/lib.rs` - static `ModelRegistry` used by
  `ghosty model list` and `ghosty model resolve`.
...
- Env: `GHOSTY_PROVIDER=<id>`; `DEEPSEEK_PROVIDER=<id>` is the legacy alias
- Config: `provider = "<id>"`
...
Ghosty never writes that file and only reads `DEEPSEEK_API_KEY`.
...
Fresh shared config writes to `~/.ghosty/config.toml`. Existing
...
Set the route with `provider = "<id>"`, `GHOSTY_PROVIDER=<id>`, or
`ghosty --provider <id>`. Set the request model with `GHOSTY_MODEL`, a
provider-specific model env var, top-level `default_text_model`, or
`[providers.<table>].model`. Set the endpoint with `GHOSTY_BASE_URL`, a
provider-specific base URL env var, or `[providers.<table>].base_url`. Set auth
with `ghosty auth set --provider <id>`, `[providers.<table>].api_key`, or
...
| `easybits` | `[providers.easybits]` | OpenAI Chat Completions (DeepSeek reseller) | `EASYBITS_API_KEY` |
...
For hosted providers, `ghosty auth set --provider <id>` saves an API key for
...
- `GHOSTY_BASE_URL` / `GHOSTY_MODEL` for the active provider.
...
model-specific**, and Ghosty fails closed: they are sent only when the
...
entry, so Ghosty does not offer one on this route.
...
`insecure_skip_tls_verify = true` key is still parsed so `ghosty doctor` can
...
runner, then point Ghosty at it with `--provider` / `/provider` or a config
table.
...
through an OpenAI-compatible API. Start DS4, then open Ghosty's prefilled,
keyless setup form:

```bash
./ds4-server --ctx 100000 --kv-disk-dir /tmp/ds4-kv --kv-disk-space-mb 8192
ghosty
# In Ghosty: /setup provider ds4
```
...
Ghosty reuses its existing OpenAI-compatible transport and DeepSeek
...
`max_tokens`, and no strict-tool mode; Ghosty follows those exact route
...
ghosty --provider ollama --model <model>
```
...
then run `ghosty auth set --provider ollama-cloud`. For ambient auth,
...
`ollama` secret slot. Ghosty does not rewrite the config, copy or delete a
...
ghosty --provider vllm --model <model>
```

vLLM's OpenAI-compatible server listens on port 8000 by default, matching
Ghosty's `VLLM_BASE_URL`.

### SGLang

```bash
python -m sglang.launch_server --model-path <model> --port 30000
ghosty --provider sglang --model <model>
```

SGLang's default port 30000 matches Ghosty's `SGLANG_BASE_URL`.
...
| `openai-codex` | Run `codex login`, then explicitly grant Ghosty read-only access to that exact credential file; no Ghosty API key is stored. |
...
| `ollama-cloud` | Create an [Ollama API key](https://ollama.com/settings/keys), save it with `ghosty auth set --provider ollama-cloud`, or set `OLLAMA_CLOUD_API_KEY` / `OLLAMA_API_KEY` in that precedence order. |
...
| `xai` | [xAI Console](https://console.x.ai/) for an API key, Ghosty-owned device login, or explicitly consented read-only Grok CLI credentials. |
| `mistral` | [Mistral Console (la Plateforme)](https://console.mistral.ai/api-keys) |
| `google` | [Google AI Studio](https://aistudio.google.com/apikey) — Ghosty uses the official Gemini OpenAI-compatible endpoint and never reads Google OAuth files. |
| `antigravity` | Sign in with the official `agy` CLI (1.1.13). Ghosty can read that login's OAuth token read-only from the exact pinned `state.vscdb` after `ghosty auth external-consent`; it never writes or refreshes the file. An `ANTIGRAVITY_API_KEY` or the process's `AGY_ADC_AUTH` wins over the file. The cloud-code wire protocol is not implemented: requests fail closed with an actionable message — use `google` for Gemini models. |
...
secret. Ghosty links straight to that console and accepts the copied key.
It never probes or impersonates `kimi_cli`/`kimi_code_cli`; first-class Kimi
OAuth remains blocked on a vendor-registered Ghosty identity.
...
Ghosty currently supports exact-path, provider-scoped **read-only** grants
...
grok login
ghosty auth external-consent --provider xai --mode read-only

ghosty auth status --provider openai-codex
...
yet preserve every unknown external schema field safely. Ghosty-started xAI
device login instead atomically activates a Ghosty-owned generation named
`$GHOSTY_HOME/credentials/xai-auth-<generation>.json`, stores only that
...
`ghosty auth external-consent --provider deepseek --mode read-only` grants
exact-path read access to `DEEPSEEK_API_KEY` in `$DSH_HOME/.credentials.yaml`
(or `~/.dsh/.credentials.yaml`), which Ghosty never writes, refreshes, or
loads into the process environment. This is separate from the DSH *harness*
integration (`ghosty integrations dsh …`, see
[INTEGRATIONS_DSH.md](INTEGRATIONS_DSH.md)), which never touches credentials
in either direction: it pins Ghosty's route identity into a `--patch`
...
| `xiaomi-mimo` | `[providers.xiaomi_mimo]` | `XIAOMI_MIMO_TOKEN_PLAN_API_KEY`, `MIMO_TOKEN_PLAN_API_KEY`, `XIAOMI_MIMO_API_KEY`, `XIAOMI_API_KEY`, `MIMO_API_KEY` | `XIAOMI_MIMO_BASE_URL`, `MIMO_BASE_URL`, `XIAOMI_MIMO_MODE`, `MIMO_MODE`; default `https://token-plan-sgp.xiaomimimo.com/v1` | Chat: `mimo-v2.5-pro`, `mimo-v2.5-pro-ultraspeed`, `mimo-v2.5`; speech/TTS: `mimo-v2.5-tts`, `mimo-v2.5-tts-voicedesign`, `mimo-v2.5-tts-voiceclone`, `mimo-v2-tts` | Xiaomi MiMo OpenAI-compatible chat completions route. `mimo-v2.5-pro` and `mimo-v2.5` can use the documented provider-native web-search plugin; ultraspeed, speech/TTS, and custom-compatible routes do not inherit it. Token Plan keys (`tp-...`) use `api-key` auth and the token-plan endpoint by default; pay-as-you-go mode uses standard API keys (`sk-...`) and `https://api.xiaomimimo.com/v1`. It sends `max_completion_tokens` and uses MiMo's `thinking` field for reasoning control. Token Plan cost/usage is credit/quota based; Ghosty shows it as unknown until Xiaomi exposes a reliable balance API. `ghosty speech` / `tts` uses the TTS models. |
| `novita` | `[providers.novita]` | `NOVITA_API_KEY` | `NOVITA_BASE_URL`; default `https://api.novita.ai/openai/v1` | `deepseek/deepseek-v4-pro`, `deepseek/deepseek-v4-flash` | OpenAI-compatible hosted route for DeepSeek model IDs. Use config or `GHOSTY_MODEL` / `DEEPSEEK_MODEL` for model overrides. |
...
| `xai` | `[providers.xai]` | `XAI_API_KEY`, Ghosty-owned device OAuth, or explicit read-only Grok CLI consent | `XAI_BASE_URL`; default `https://api.x.ai/v1` | `grok-4.6` (default), `grok-4.5`, `grok-4.3`, `grok-build`, `grok-composer-2.5-fast`, `grok-4.20-0309-reasoning`, `grok-4.20-0309-non-reasoning` | xAI/Grok OpenAI-compatible Chat Completions route. Grok 4.6 has a 500K context window, text/image input, function calls, structured output, server-side web search, and `low`/`medium`/`high`/`xhigh` reasoning (default `high`). Its standard rates double when the prompt reaches 200K tokens; the same 2x long-context rule applies to `grok-4.5` (500K context, $2.00 / $0.30 cached / $6.00) and `grok-4.3` (1M context, $1.25 / $0.20 cached / $2.50) per their [model pages](https://docs.x.ai/docs/models/grok-4.5). There is no documented `latest`/`fast` alias and no published numeric output limit. **API-key** (default): Bearer token from console.x.ai via `XAI_API_KEY` / keyring / `api_key`. **OAuth**: `ghosty auth xai-device` uses SSH-friendly device login and Ghosty-owned storage, which may refresh itself. Existing Grok CLI credentials require `ghosty auth external-consent --provider xai --mode read-only`; the granted external file is never refreshed or rewritten. OAuth may return HTTP 403 on some SuperGrok tiers — keep API-key as the reliable fallback. `XAI_MODEL` is accepted. Provider aliases: `x-ai`, `x_ai`, `grok`. |
...
Ghosty's `huggingface` provider ID is only the OpenAI-compatible chat
inference route through Hugging Face Inference Providers. It is selected with
`/provider huggingface`, `GHOSTY_PROVIDER=huggingface`, or
...
`ghosty --provider xiaomi-mimo speech "text" --model tts` (or the `tts`
...
Plan docs and console expose credit/quota semantics, but Ghosty does not
...
includes `mimo-v2.5` for image input. Ghosty exposes image analysis through the
...
Ghosty preserves the `deepseek/` wire-model prefix under the OpenRouter
...
fields, Ghosty treats them as absent for that response rather than as a
...
same models, same reasoning behavior. Since 0.0.15 `easybits` is a first-class
`ProviderKind` (no longer an alias to `deepseek`): the base URL defaults to
`https://www.easybits.cloud/api/v2/llm/v1`, the credential lives in
`[providers.easybits]`, and the model catalog, effort ladder and endpoint roster
are shared with DeepSeek. In the route resolver EasyBits is classified as a
reseller (pass-through), not a strict direct provider. One EasyBits key
(`eb_sk_live_...`) serves both the LLM provider and the EasyBits MCP server.

**Setup:**

```bash
ghosty auth set --provider easybits --api-key eb_sk_live_...
```

On a fresh install this also makes `easybits` the active provider. On an
existing config it only stores the key; switch with `/provider` (session) and
`/model save-default` (persist).
...
**Tool budget.** DeepSeek's API caps requests at **128 tools** and counts every
advertised tool (it ignores `defer_loading`). EasyBits exposes `core` (65) and
`sandbox` (47); with Ghosty's ~36 built-ins only one group fits at a time.
Ghosty warns before sending an oversized request. For boxes *and* S3/DB in the
same turn, connect only `?tools=sandbox` over MCP and call the REST API for the
rest — see [`TALLER.md`](TALLER.md).

**Footgun — env overrides config.** `GHOSTY_PROVIDER` / legacy
`DEEPSEEK_PROVIDER` override `provider = "easybits"` from the config file. A
stray `export DEEPSEEK_PROVIDER=deepseek` silently flips the active provider;
`unset DEEPSEEK_PROVIDER GHOSTY_PROVIDER` or open a fresh shell.

## Static Model Registry

`ghosty model list` and `ghosty model resolve` use the static registry in
`crates/agent/src/lib.rs`. This is not the same as live `/models` discovery.
Use `/models` or `ghosty models` to fetch model IDs from the active API
...
| `xiaomi-mimo` | `mimo-v2.5-pro`, `mimo-v2.5-pro-ultraspeed`, `mimo-v2.5`; speech/TTS IDs are selected through `ghosty speech` / `tts` | yes | yes for chat models; no for speech/TTS models |
...
`ghosty doctor --json` exposes the `capability` object. It is static
...
MiniMax M3 uses input-length and service tiers. Ghosty omits
...
disabled. Ghosty sends explicit controls when the user selects a reasoning
mode.
...
Ghosty only executes tools when the provider returns Chat Completions
...
- If in doubt, test a small `File` `read` or `search_content` action against a
  known tool-calling model before debugging Ghosty's tool registry.

Changing `provider`, `base_url`, or `model` can select a route that supports the
OpenAI-compatible payload shape, but Ghosty cannot convert arbitrary JSON
...
15:59 UTC. Ghosty migrates either name to `deepseek-v4-flash` before a
...
- **Direct Moonshot K3** (exact `https://api.moonshot.ai/v1` with
  ...
  The direct platform publishes no `off` state for K3, and Ghosty will not
...
Normal dispatched
`auto` uses Ghosty's auto-reasoning selector and sends a concrete tier;
...
- A unified `Provider` trait in `ghosty-agent` that owns env precedence,
