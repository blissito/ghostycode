...
Ghosty Code is built for open-source and open-weight coding models. DeepSeek V4
...
Ghosty Code inspect itself and return a small, reviewable improvement.
...
> easy to inspect. Ghosty Code uses the metaphor in the same practical sense: the
...
> 大比例模型让结构更容易被观察和理解。Ghosty Code 借用这个比喻：智能体可以进行大量
...
> Ghosty Code はこの比喩を実務的に使います。エージェントはキャッシュ、ツール、サブ
...
Paste this into Ghosty Code from the repository root:

```text
You are running inside Ghosty Code on DeepSeek V4 Pro.

Your task is to improve Ghosty Code itself by finding exactly one small,
...
从仓库根目录把这段粘贴到 Ghosty Code：

```text
你正在 DeepSeek V4 Pro 驱动的 Ghosty Code 中运行。

你的任务是改进 Ghosty Code 本身：只找一个很小、可审查的点，看看这个
...
リポジトリのルートで、このプロンプトを Ghosty Code に貼り付けます。

```text
あなたは DeepSeek V4 Pro 上の Ghosty Code の中で動いています。

目的は Ghosty Code 自体を改善することです。ただし、対象はひとつだけに
