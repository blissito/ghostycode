...
- [ ] Run `./scripts/release/prepare-release.sh X.Y.Z` — it bumps the
      ...
      (`version` + `ghostyBinaryVersion`), Runtime SDK, VS Code extension
...
- [ ] After the final source reaches `main`, dispatch exact-head full CI and the
      ...
      arm64, Windows arm64, `ghosty-tui`, the NSIS installer, archives, checksum
      manifests, and seven compatibility-only `ghosty-tui-*` release
...
- [ ] The live GitHub Release body has its own `## Contributors` or
      ...
      ```
      gh release view vX.Y.Z --repo blissito/ghostycode --json body \
...
- [ ] Distribution channels are canonical-first: the website install page
      (ghosty.net/install) shows Ghosty-native commands first (`npm install -g
      ghosty`, `curl .../install.sh | sh`); Homebrew is labeled as legacy
      compatibility; the shell installer uses ghosty-native names as documented
      in `docs/REBRAND.md#homebrew`.
- [ ] `crates.io` has the new version (or the `publish-crates.sh` job has
      pushed it).
- [ ] `ghcr.io/blissito/ghostycode:vX.Y.Z` and `:latest` are updated.
