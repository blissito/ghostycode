# `ghosty remote-setup` — Design & Implementation Plan
...
One command — `ghosty remote-setup` — that guides a user through standing up
a remote Ghosty Code agent they can talk to from a phone chat app, across:
...
same problem and **validates this design**. Use it for ideas; keep Ghosty Code's
...
- **Provider = a `runtime.env` concern.** The runtime resolves the provider from
  `GHOSTY_PROVIDER` and the provider's own key var. The bridge never needs to
...
```rust
/// Provision and configure a remote Ghosty Code agent (cloud + chat bridge).
...
| `--out <dir>` | Bundle output dir (default `./ghosty-deploy/<cloud>-<bridge>`). |
...
pub struct BridgeSpec {
    ...
    pub service_unit: &'static str,    // "ghosty-telegram-bridge.service"
...
4. **Secrets** — prompt for: provider API key, bridge token(s) from
   `secret_keys`, allowlist (chat ids). Generate a random `GHOSTY_RUNTIME_TOKEN`.
...
Written to `./ghosty-deploy/<cloud>-<bridge>/`:
...
- `runtime.env` — **provider config lives here**:
  ...
  RUST_LOG=info
  ```
- `<bridge>.env` — transport only: `GHOSTY_RUNTIME_URL=http://127.0.0.1:7878`,
  matching `GHOSTY_RUNTIME_TOKEN`, allowlist, `TELEGRAM_BOT_TOKEN` (or Feishu
  app id/secret), `GHOSTY_WORKSPACE`, `GHOSTY_MODEL`.
- `ghosty-runtime.service`, `ghosty-<bridge>.service`.
...
3. `az vm create` with `--custom-data cloud-init.yaml` and a **system-assigned
   managed identity**; cloud-init pulls `ghcr.io/hmbown/ghosty:latest`, reads
   the secrets from Key Vault via the identity, writes `/etc/ghosty/*.env`,
...
## Namespace migration: `DEEPSEEK_*` → `GHOSTY_*`

Follow the convention already in `crates/config/src/lib.rs`: **read
`GHOSTY_X` first, fall back to `DEEPSEEK_X`.** Nothing breaks for existing
deployments.

Touch list:
1. **Bridges** (`integrations/feishu-bridge`, `integrations/telegram-bridge`):
   in `lib.mjs`/`index.mjs`, read `process.env.GHOSTY_X ?? process.env.DEEPSEEK_X`
   ...
   Validators accept either; templates emit `GHOSTY_*`.
2. **Deploy units** (`deploy/tencent-lighthouse/systemd/*`,
   `integrations/*/deploy/*`): `DEEPSEEK_RUNTIME_*` → `GHOSTY_RUNTIME_*`,
   env file paths `/etc/deepseek/` → `/etc/ghosty/` (keep reading the old path
   if present).
3. **`.env.example` files + `config.example.toml`**: lead with `GHOSTY_*`,
...
4. **Drop DeepSeek-shaped defaults** in the bridge: no hardcoded
   ...
   `GHOSTY_PROVIDER` + the registry's key var.
...
be converted to `GHOSTY_*` first as the reference implementation.
...
- `bundle.rs`: rendering a bundle for each cloud×bridge×provider triple produces
  files with `GHOSTY_*` keys, a matching runtime/bridge token, and a non-empty
  RUNBOOK.
...
1. **Now (safe, untracked):** convert the new Telegram bridge to `GHOSTY_* ??
