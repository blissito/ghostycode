...
Ghosty Code uses the same branching primitive at three scales:
...
The maintainer chooses the frontier point. Ghosty Code should not collapse
...
- Add `ghosty replay <turn_id>` for deterministic replay.
...
- Build the first offline compile pipeline: Rust harvests trainsets, a Python
  sidecar runs the optimizer, and Ghosty Code emits a reviewed Module JSON
  artifact.
...
- Add whale-species module passports, for example
  `Sei: ghosty-agent-prompt.v0.10.0-gepa-1`.
