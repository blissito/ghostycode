...
ghosty exposes a local runtime API through `ghosty serve --http` and
machine-readable health via `ghosty doctor --json`. It also exposes
`ghosty serve --acp` for editor clients that speak the Agent Client Protocol
...
| `skills.global.path` / `.present` / `.count` | — | Ghosty Code global skills dir (`~/.ghosty/skills`, with legacy `~/.deepseek/skills` support) |
...
## HTTP/SSE runtime API: `ghosty serve --http`

```bash
ghosty serve --http [--host 127.0.0.1] [--port 7878] [--workers 2] [--auth-token TOKEN]
ghosty serve --mobile [--host 0.0.0.0] [--port 7878] [--auth-token TOKEN]
```
