...
Ghosty provides an Android arm64 build and archive path for
...
Or, when a release includes `ghosty-android-arm64.tar.gz`, extract it
into `$PREFIX/bin`.

> **Do not** install the GNU libc `ghosty-linux-arm64` archive in Termux.
> Android uses Bionic libc, not glibc — the Linux binary will not run.

## Platform behavior on Android

Ghosty's security model has three distinct layers on Android:
...
1. **Android's app sandbox** — Android assigns Termux its own app UID and
   ...
   Ghosty inherit that app boundary and any storage or other permissions the
...
2. **Ghosty's per-command sandbox backend** — Seatbelt (macOS) or the
   opt-in bubblewrap wrapper (Linux) can further narrow what a child command
   may access. Ghosty does not currently provide that additional layer on
   Android.
3. **Ghosty's own gates** — workspace trust, approval prompts,
...
### Ghosty sandbox backend: none

Ghosty's existing Seatbelt and Linux bubblewrap integrations do not target
Android. Consequently, `ghosty doctor --json` reports the sandbox as
`{"available": false, "kind": null}` on Android. That status describes the
absence of an additional Ghosty child-process sandbox; it does not mean
...
- Shell commands retain Termux's Android app boundary but receive no
  Ghosty-specific filesystem narrowing. Treat every location available to
  Termux, including user-granted shared storage, as potentially available to a
  command that you approve.

### Approvals: still apply

Ghosty's approval system (interactive prompts for risky actions,
...
Ghosty's Termux/native build has no supported OS keyring backend (the
desktop Secret Service/dbus integration is unavailable, and Ghosty does not
...
`~/.ghosty/secrets/` (Termux home directory), protected only by `0600`
...
- Keys saved through setup, `/provider`, or `ghosty auth set` are written to
  `~/.ghosty/config.toml` and mirrored to
  `~/.ghosty/secrets/secrets.json`. Treat both as plaintext sensitive
  files.
- `ghosty auth status --provider <id>` reports which secret backend is
  active for a provider.

### Self-update

`ghosty update` on Android requests the `ghosty-android-arm64`
...
| Ghosty command sandbox | ❌ unavailable | No bubblewrap/Seatbelt backend on Android |
| Ghosty keyring backend | ❌ unavailable | Falls back to file-backed secrets |
...
| Shell execution | ⚠️ app boundary only | No Ghosty-specific narrowing; runtime QA pending |
