...
Workrooms are GhostyCode's chat-native abstraction for durable, addressable
...
1. **Creation.** A future workroom is created when a thread is started with a
   ...
   is stable and can be shared as a `ghosty://workroom/...` link.
...
3. **Link resolution.** When a `ghosty://workroom/...` link appears in
...
Persisted workroom state should live alongside existing GhostyCode state:
...
|---|---|
| `ghosty-protocol` | Types: `Workroom`, `WorkroomId`, `WorkroomThread`, `WorkroomEvent`, `WorkroomLink`, `ExternalThreadRef`, `AgentAttribution` |
| `ghosty-app-server` | Future endpoints: `GET /workrooms`, `GET /workroom/:id/threads`, `GET /workroom/resolve` |
| `ghosty-tui` | Future model-facing link resolution and optional sidebar inbox |
| `ghosty-state` | Future: persistent workroom store (Phase 2) |
