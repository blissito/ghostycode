...
`ghosty-tui` includes an opt-in capacity-aware context controller. In the
...
- otherwise `~/.ghosty/memory/<session_id>.jsonl`
- fallback: existing `~/.deepseek/memory/<session_id>.jsonl` or workspace-local `.ghosty` / legacy `.deepseek` memory paths when needed
