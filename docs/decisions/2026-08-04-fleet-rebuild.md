...
- per-role profile files (`~/.ghosty/agents/*.toml`, `.ghosty/agents/*.toml`,
...
- named fleet files (`fleets/<name>.toml` at `$GHOSTY_HOME` or workspace)
...
- scope + source: personal (`$GHOSTY_HOME/fleets/`) or workspace
  (`.ghosty/fleets/`), the exact file path shown in the UI.
...
- `$GHOSTY_HOME/fleets/selected` — user-global default Fleet (plain text name).
- `.ghosty/fleets/selected` — workspace selection, intentional and labeled.
...
$GHOSTY_HOME/credentials/*). The "authorized here, locked there" failure is
a config-file-selection problem: an explicit --config/GHOSTY_CONFIG_PATH
