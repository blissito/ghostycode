# Ghosty v0.9.11 local non-benchmark acceptance contract
...
```text
GHOSTY_BUILD_SHA=<full-sha> cargo build --release --locked \
...
retaining the rollback backup, then verify `ghosty` and `ghosty-tui`. If tracked
