...
### 1.1 `ghosty-mcp` crate (`crates/mcp/`)
...
- **MCP stdio server** — a minimal MCP server that exposes Ghosty Code's own tools
  over stdio for external MCP clients. Used by the `ghosty mcp` CLI subcommand.
...
- The `ghosty` CLI (non-interactive mode)
...
**Dependencies:** `serde`, `serde_json`, `ghosty-protocol` (for tool schema)
...
2. Re-export from `ghosty-mcp` for backward compatibility
3. Update `Cargo.toml` in `ghosty-mcp` to depend on `ghosty-mcp-protocol`
...
3. Update `ghosty-tui` to depend on `ghosty-mcp-client`
4. Update `ghosty-cli` to depend on `ghosty-mcp-server`
5. Deprecate `ghosty-mcp` crate (re-exports from new crates)
...
2. Encrypted file in `~/.ghosty/mcp-credentials/` (fallback)
...
| Breaking internal imports | Phase 2 carries `ghosty-mcp` deprecation shim for one release |
