...
1. Pull the queue (read everything, decide nothing yet):
   ```bash
   gh pr list --repo blissito/ghostycode --state open --limit 200 \
   ...
   gh issue list --repo blissito/ghostycode --state open --limit 300 \
...
3. Confirm each shortlisted PR from code, tests, comments, and checks:
   ...
   gh pr checks N --repo blissito/ghostycode
   ```
