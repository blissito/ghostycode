...
harness_module = importlib.import_module("ghosty_harness.harness")

GhostyHarness = harness_module.GhostyHarness
GhostyHarnessConfig = harness_module.GhostyHarnessConfig
...
class FakeRuntime:
    ...
    async def run(self, argv, env):
        ...
        if argv[-1:] == ["--version"]:
            return ProgramResult(0, "ghosty 0.9.1 (candidate)", "")
...
class HarnessTests(unittest.IsolatedAsyncioTestCase):
    async def test_preinstalled_candidate_requires_exact_version(self):
        config = GhostyHarnessConfig(
            version="0.9.1", binary_path="/candidate/ghosty"
        )
        runtime = FakeRuntime()

        await GhostyHarness(config).setup(runtime)

        self.assertEqual(runtime.runs, [(["/candidate/ghosty", "--version"], {})])

    async def test_launch_isolates_config_routes_interception_and_keeps_safe_receipt(self):
        config = GhostyHarnessConfig(
            version="0.9.1",
            binary_path="/candidate/ghosty",
            max_turns=17,
        ...
        await GhostyHarness(config).launch(
            ctx,
            rollout,
            runtime,
        ...
        self.assertEqual(argv[0], "/candidate/ghosty")
        ...
        self.assertEqual(env["GHOSTY_PROVIDER"], "openai")
        self.assertEqual(env["GHOSTY_MODEL"], "test/model")
        self.assertEqual(env["GHOSTY_TELEMETRY"], "false")
        self.assertEqual(env["GHOSTY_ALLOW_INSECURE_HTTP"], "1")
        self.assertEqual(env["CALLER_SETTING"], "kept")
        self.assertEqual(len(runtime.writes), 1)
        mcp_path, mcp_bytes = runtime.writes[0]
        self.assertEqual(mcp_path, env["GHOSTY_MCP_CONFIG"])
        self.assertTrue(mcp_path.startswith(".vf-ghosty/"))
    ...
    async def test_isolated_runtime_uses_external_sandbox_without_elevation_flag(self):
        config = GhostyHarnessConfig(binary_path="/candidate/ghosty")
        runtime = FakeRuntime(
            runtime_type="prime",
        ...
        self.assertNotIn("GHOSTY_ALLOW_INSECURE_HTTP", env)
    ...
    def test_exact_version_rejects_prerelease_and_longer_tokens(self):
        ...
        self.assertTrue(has_version("ghosty 0.9.1 (abc123)", "0.9.1"))
        self.assertFalse(has_version("ghosty 0.9.1-rc.1", "0.9.1"))
        self.assertFalse(has_version("ghosty 0.9.10", "0.9.1"))

    async def test_invalid_config_rejects_shell_and_tool_injection(self):
        for config in [
            GhostyHarnessConfig(version="0.9.1; touch /tmp/pwned"),
            GhostyHarnessConfig(disabled_tools=["read_file,exec_shell"]),
            GhostyHarnessConfig(max_turns=0),
        ]:
            with self.assertRaises(ValueError):
                await GhostyHarness(config).setup(FakeRuntime())

    async def test_install_is_pinned_and_checksum_verified(self):
        config = GhostyHarnessConfig(version="0.9.1")
        runtime = FakeRuntime()
        ...
        self.assertIn("ghosty-artifacts-sha256.txt", command)
