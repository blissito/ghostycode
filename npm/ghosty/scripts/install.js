function assertSupportedNode() {
  ...
  if (Number.isNaN(major) || major < 18) {
    process.stderr.write(
      "ghosty: Node.js 18 or newer is required for npm installation. " +
      `Current Node.js version is ${version}. ` +
      "Please upgrade Node.js and rerun `npm install -g ghosty`.\n",
    );
    process.exit(1);
  }
}
...
// `ghostyBinaryVersion` lets a packaging-only npm release target a specific
// GhostyCode binary; legacy env vars and `deepseekBinaryVersion` stay supported
...
function binaryPaths() {
  ...
  return {
    ghosty: {
      asset: ghosty,
      target: path.join(releaseDir, process.platform === "win32" ? "ghosty.exe" : "ghosty"),
    },
  };
...
function logInfo(message) {
  ...
  process.stderr.write(`ghosty: ${message}\n`);
}
...
function installFailureHint(error) {
  ...
  if (releaseBase) {
    return [
      "ghosty install hint:",
      `  GHOSTY_RELEASE_BASE_URL resolves to ${releaseBase}`,
      "  Verify that this directory contains ghosty-artifacts-sha256.txt",
      "  plus the ghosty/ghosty-tui binary assets for your platform (single binary).",
    ].join("\n");
  }
  ...
  return [
    ...
    "    GHOSTY_RELEASE_BASE_URL=https://<mirror>/<release-asset-directory>/",
    "  or GHOSTY_USE_CNB_MIRROR=1 on Linux x64.",
    "  The directory must contain ghosty-artifacts-sha256.txt and the platform binaries.",
...
function createProgressReporter(assetName, totalBytes) {
  ...
  const render = (final) => {
    if (totalBytes && totalBytes > 0) {
      ...
      const line = `ghosty: downloading ${assetName}: ${formatMb(received)} / ${formatMb(totalBytes)} MB (${pct}%)`;
      if (interactive) {
      ...
      const line = `ghosty: downloading ${assetName}: ${formatMb(received)} MB downloaded`;
      if (interactive) {
        process.stderr.write(`${line}\r`);
      } else {
        process.stderr.write(`${line}\n`);
  ...
  return {
    ...
    finish() {
      ...
      process.stderr.write(`ghosty: ${assetName} ... done.\n`);
    },
  };
}
    ...
    if (totalTimeoutMs > 0) {
      totalTimer = setTimeout(() => {
        fail(new DownloadTimeoutError(
          ...
          `(set GHOSTY_DOWNLOAD_TIMEOUT_MS to raise it; current stall budget is ${stallMs} ms)`,
        ));
      }, totalTimeoutMs);
    }
    ...
    const armStallTimer = () => {
      ...
      stallTimer = setTimeout(() => {
        fail(new DownloadTimeoutError(
          ...
          `(set GHOSTY_DOWNLOAD_STALL_MS to raise it; total budget is ${totalTimeoutMs} ms)`,
        ));
      }, stallMs);
    };
...
async function selectReleaseSource(options) {
  ...
  const repo = options.repo || "blissito/ghostycode";
...
async function run(options = {}) {
  ...
  const getSource = () => {
    if (!sourcePromise) {
      sourcePromise = selectReleaseSource({
        version,
        repo,
        requiredAssets: [paths.ghosty.asset],
        context,
        env,
  ...
  await ensureBinary(paths.ghosty.target, paths.ghosty.asset, version, repo, getChecksums, {
    context,
    getSource,
...
async function getBinaryPath(name) {
  ...
  if (name === "ghosty") {
    return paths.ghosty.target;
  }
  if (name === "ghosty-tui") {
    return paths.ghostyTui.target;
  }
  if (name === "ghosty-tui") {
    // v0.9.5 single-binary: ghosty-tui is now an alias to ghosty for backwards compat
    return paths.ghosty.target;
  }
...
if (require.main === module) {
  run({ context: "install" }).catch((error) => {
    console.error("ghosty install failed:", error.message);
