...
function buildFromSourceHint() {
  return [
    ...
    "  bin=$(dirname \"$(command -v ghosty)\")",
    "  ln -sf \"$bin/ghosty\" \"$bin/ghosty-tui\"   # optional short alias",
    "",
    ...
    "  bin=$(dirname \"$(command -v ghosty)\")",
    "  ln -sf \"$bin/ghosty\" \"$bin/ghosty-tui\"",
    "",
...
function glibcCompatibilityMessage(required, host) {
  ...
  return [
    `Prebuilt Ghosty Linux binaries require GLIBC_${formatVersion(required)}, but ${hostLine}`,
    "",
    ...
    buildFromSourceHint(),
    "",
    "Set GHOSTY_SKIP_GLIBC_CHECK=1 to bypass this check at your own risk.",
  ].join("\n");
}
