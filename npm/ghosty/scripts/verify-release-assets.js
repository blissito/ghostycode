...
function resolveBinaryVersion() {
  const configuredVersion =
    ...
    pkg.ghostyBinaryVersion || pkg.deepseekBinaryVersion ||
    pkg.version;
...
function resolveRepo() {
  return (
    process.env.GHOSTY_GITHUB_REPO ||
...
function hasReleaseBaseOverride() {
  return Boolean(
    process.env.GHOSTY_RELEASE_BASE_URL ||
      ...
      process.env.GHOSTY_USE_CNB_MIRROR,
  );
}
...
function assertPackageVersionMatchesBinaryVersion(version) {
  ...
  if (process.env.GHOSTY_ALLOW_NPM_BINARY_MISMATCH === "1") {
    console.log(
  ...
  throw new Error(
    `npm package version ${pkg.version} does not match ghostyBinaryVersion ${version}. ` +
      "Set GHOSTY_ALLOW_NPM_BINARY_MISMATCH=1 only for an intentional packaging-only npm release.",
  );
}
  ...
  return new Promise((resolve, reject) => {
    const req = client.request(
      url,
      {
        method,
        headers: {
          "User-Agent": "ghosty-npm-release-check",
        },
      },
      (res) => {
