...
test("parseChecksumManifest accepts GNU and BSD filename forms", () => {
  const manifest = parseChecksumManifest(
    [
      "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa  ghosty-linux-x64",
      "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb *ghosty-windows-x64.exe",
    ].join("\n"),
  );

  assert.equal(manifest.get("ghosty-linux-x64"), "a".repeat(64));
  assert.equal(manifest.get("ghosty-windows-x64.exe"), "b".repeat(64));
});

test("parseChecksumManifest rejects malformed checksum rows", () => {
  assert.throws(
    () => parseChecksumManifest("not-a-sha  ghosty-linux-x64"),
...
test("assertReleaseAssetsFresh rejects missing release assets", () => {
  assert.throws(
    () =>
      assertReleaseAssetsFresh(
        { assets: [{ name: "ghosty-linux-x64", state: "uploaded", updated_at: "2026-06-26T00:10:00Z" }] },
        ["ghosty-linux-x64", "ghosty-artifacts-sha256.txt"],
...
test("assertChecksumManifestIncludes rejects missing bundle manifest and archive rows", () => {
  const manifest = parseChecksumManifest(
    `${"a".repeat(64)}  ghosty-linux-x64.tar.gz`,
  );
  ...
  assert.throws(
    () =>
      assertChecksumManifestIncludes(
        manifest,
        ["ghosty-linux-x64.tar.gz", "ghosty-bundles-sha256.txt"],
        "Canonical checksum manifest",
      ),
    /Canonical checksum manifest is missing ghosty-bundles-sha256\.txt/,
  );
});

test("bundle checksum rows use public archive basenames", () => {
  const manifest = parseChecksumManifest(
    `${"a".repeat(64)}  bundles/ghosty-linux-x64.tar.gz`,
  );
  ...
  assert.throws(
    ...
    /Bundle checksum manifest is missing ghosty-linux-x64\.tar\.gz/,
  );
});
...
test("assertReleaseAssetsFresh rejects assets older than the release workflow run", () => {
  assert.throws(
    () =>
      assertReleaseAssetsFresh(
        { assets: [{ name: "ghosty-linux-x64", state: "uploaded", updated_at: "2026-06-25T23:59:59Z" }] },
        ["ghosty-linux-x64"],
        { database_id: 123, created_at: "2026-06-26T00:00:00Z" },
      ),
    /asset set is stale/,
...
test("assertReleaseAssetsFresh rejects non-uploaded assets", () => {
  assert.throws(
    () =>
      assertReleaseAssetsFresh(
        { assets: [{ name: "ghosty-linux-x64", state: "new", updated_at: "2026-06-26T00:10:00Z" }] },
        ["ghosty-linux-x64"],
        { database_id: 123, created_at: "2026-06-26T00:00:00Z" },
      ),
    /asset set is stale/,
...
test("assertReleaseAssetsFresh accepts assets uploaded before a job-level rerun bumped run_started_at (#5429)", () => {
  assert.doesNotThrow(() =>
    assertReleaseAssetsFresh(
      { assets: [{ name: "ghosty-linux-x64", state: "uploaded", updated_at: "2026-08-15T02:00:00Z" }] },
      ["ghosty-linux-x64"],
      {
        database_id: 123,
...
test("assertReleaseAssetsFresh still rejects assets older than the successful release job", () => {
  assert.throws(
    () =>
      assertReleaseAssetsFresh(
        { assets: [{ name: "ghosty-linux-x64", state: "uploaded", updated_at: "2026-08-15T00:30:00Z" }] },
        ["ghosty-linux-x64"],
        {
          database_id: 123,
          run_started_at: "2026-08-15T10:00:00Z",
...
test("assertPackageVersionMatchesBinaryVersion allows packaging-only releases only with an explicit override", () => {
  ...
  assert.throws(
    ...
    /does not match ghostyBinaryVersion/,
  );

  const previous = process.env.GHOSTY_ALLOW_NPM_BINARY_MISMATCH;
  process.env.GHOSTY_ALLOW_NPM_BINARY_MISMATCH = "1";
  try {
    assert.doesNotThrow(() => assertPackageVersionMatchesBinaryVersion("0.0.0-packaging-test"));
  } finally {
    if (previous === undefined) {
      delete process.env.GHOSTY_ALLOW_NPM_BINARY_MISMATCH;
    } else {
      process.env.GHOSTY_ALLOW_NPM_BINARY_MISMATCH = previous;
    }
  }
});
...
test("full local release fixture satisfies the public asset inventory", () => {
  ...
  const fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "ghosty-assets-"));
  ...
  try {
    ...
    fs.writeFileSync(
      path.join(buildDir, `ghosty${executableSuffix}`),
      "fixture:ghosty\n",
    );
    ...
    for (const legacyAsset of LEGACY_TUI_BRIDGE_ASSET_NAMES) {
      const primaryAsset = legacyAsset.replace("ghosty-tui-", "ghosty-");
      assert.deepEqual(
