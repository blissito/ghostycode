...
test("version flags prefer the installed binary over package metadata", async () => {
  ...
  await run("ghosty", {
    args: ["--version"],
    getBinaryPath: async () => "/tmp/ghosty-test-binary",
    spawnSync: (binary, args, options) => {
      spawned = true;
      assert.equal(binary, "/tmp/ghosty-test-binary");
...
test("ghosty-tui wrapper dispatches the native shortcut binary", async () => {
  const resolvedNames = [];
  const spawned = [];

  await run("ghosty-tui", {
    args: ["--version"],
    getBinaryPath: async (name) => {
      resolvedNames.push(name);
      return "/tmp/ghosty-tui-test-binary";
    },
  ...
  assert.deepEqual(resolvedNames, ["ghosty-tui"]);
  assert.deepEqual(spawned, [
    { binary: "/tmp/ghosty-tui-test-binary", args: ["--version"] },
  ]);
});
