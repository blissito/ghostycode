function assertSupportedNode() {
  ...
  if (Number.isNaN(major) || major < 18) {
    process.stderr.write(
      ...
      "Please upgrade Node.js and rerun `npm install -g ghostycode`.\n",
    );
    process.exit(1);
  }
}
...
function resolveRepo() {
  return process.env.DEEPSEEK_TUI_GITHUB_REPO || process.env.DEEPSEEK_GITHUB_REPO || "blissito/ghostycode";
}
...
function binaryPaths() {
  ...
  return {
    ...
    tui: {
      asset: tui,
      target: path.join(releaseDir, process.platform === "win32" ? "ghosty-tui.exe" : "ghosty-tui"),
    },
  };
}
...
function installFailureHint(error) {
  ...
  if (releaseBase) {
    return [
      ...
      "  plus the ghosty/ghosty-tui binary assets for your platform.",
    ].join("\n");
  }
...
async function run(options = {}) {
  ...
  await Promise.all([
    ensureBinary(paths.ghosty.target, paths.ghosty.asset, version, repo, getChecksums, { context }),
