...
if [[ -z "${RUNTIME_ENV:-}" ]]; then
  if [[ -f /etc/ghosty/runtime.env || ! -f /etc/deepseek/runtime.env ]]; then
    RUNTIME_ENV="/etc/ghosty/runtime.env"
  else
    RUNTIME_ENV="/etc/deepseek/runtime.env"
  fi
fi
REPO_ROOT="${REPO_ROOT:-${WHALEBRO_ROOT}/ghosty}"
BRIDGE_KIND="${GHOSTY_BRIDGE:-${DEEPSEEK_BRIDGE:-feishu}}"

case "${BRIDGE_KIND}" in
  feishu|lark)
    if [[ -z "${BRIDGE_ENV:-}" ]]; then
      if [[ -f /etc/ghosty/feishu-bridge.env || ! -f /etc/deepseek/feishu-bridge.env ]]; then
        BRIDGE_ENV="/etc/ghosty/feishu-bridge.env"
      else
        BRIDGE_ENV="/etc/deepseek/feishu-bridge.env"
      fi
    fi
    BRIDGE_DIR="${BRIDGE_DIR:-${GHOSTY_ROOT}/bridge}"
    BRIDGE_UNIT="${BRIDGE_UNIT:-ghosty-feishu-bridge}"
    BRIDGE_PACKAGE="${BRIDGE_PACKAGE:-integrations/feishu-bridge}"
    ;;
  telegram)
    if [[ -z "${BRIDGE_ENV:-}" ]]; then
      if [[ -f /etc/ghosty/telegram-bridge.env || ! -f /etc/deepseek/telegram-bridge.env ]]; then
        BRIDGE_ENV="/etc/ghosty/telegram-bridge.env"
      else
        BRIDGE_ENV="/etc/deepseek/telegram-bridge.env"
      fi
    fi
    BRIDGE_DIR="${BRIDGE_DIR:-${GHOSTY_ROOT}/telegram-bridge}"
    BRIDGE_UNIT="${BRIDGE_UNIT:-ghosty-telegram-bridge}"
...
check_env() {
  ...
  runtime_token="$(env_value_any "${RUNTIME_ENV}" GHOSTY_RUNTIME_TOKEN DEEPSEEK_RUNTIME_TOKEN)"
  bridge_token="$(env_value_any "${BRIDGE_ENV}" GHOSTY_RUNTIME_TOKEN DEEPSEEK_RUNTIME_TOKEN)"
  workspace="$(env_value_any "${BRIDGE_ENV}" GHOSTY_WORKSPACE DEEPSEEK_WORKSPACE)"
  provider="$(env_value_any "${RUNTIME_ENV}" GHOSTY_PROVIDER DEEPSEEK_PROVIDER)"

  if [[ "${BRIDGE_KIND}" == "telegram" ]]; then
    allow_groups="$(env_value "${BRIDGE_ENV}" TELEGRAM_ALLOW_GROUPS)"
    allow_unlisted="$(env_value_any "${BRIDGE_ENV}" TELEGRAM_ALLOW_UNLISTED GHOSTY_ALLOW_UNLISTED DEEPSEEK_ALLOW_UNLISTED)"
  else
    domain="$(env_value "${BRIDGE_ENV}" FEISHU_DOMAIN)"
    allow_groups="$(env_value "${BRIDGE_ENV}" FEISHU_ALLOW_GROUPS)"
    allow_unlisted="$(env_value_any "${BRIDGE_ENV}" GHOSTY_ALLOW_UNLISTED DEEPSEEK_ALLOW_UNLISTED)"
  fi
...
check_validator() {
  ...
  local runner=(node)
  if [[ "${EUID}" -eq 0 ]] && id -u "${GHOSTY_USER}" >/dev/null 2>&1 && have_command sudo; then
    runner=(sudo -u "${GHOSTY_USER}" node)
  fi
...
check_systemd() {
  ...
  for unit in ghosty-runtime "${BRIDGE_UNIT}"; do
...
check_localhost_health() {
  ...
  local port token
  port="$(env_value_any "${RUNTIME_ENV}" GHOSTY_RUNTIME_PORT DEEPSEEK_RUNTIME_PORT)"
  port="${port:-7878}"
  token="$(env_value_any "${BRIDGE_ENV}" GHOSTY_RUNTIME_TOKEN DEEPSEEK_RUNTIME_TOKEN)"
