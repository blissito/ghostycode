...
ghosty auth set --provider deepseek --api-key sk-...

# Method 3: config.toml
# Add to ~/.ghosty/config.toml:
api_key = "sk-..."

# Check what's active:
ghosty auth status    # shows config, keyring, and env-var state
ghosty doctor         # full connectivity check`}
        </pre>
        <p>
          Saved config keys take precedence over environment variables.
          Use <code className="inline">ghosty auth clear --provider deepseek</code> to remove a saved key.
        </p>
      </>
    ),
...
brew update && brew upgrade ghosty`}
        ...
        <p>
          ...
          If a mirror is lagging, download directly from <a href="https://github.com/blissito/ghostycode/releases" className="body-link">GitHub Releases</a>.
        </p>
      </>
    ),
...
# 方法 3：config.toml
# 在 ~/.ghosty/config.toml 中添加：
api_key = "sk-..."

# 查看当前状态：
ghosty auth status    # 显示配置、密钥环和环境变量状态
ghosty doctor         # 完整连接检查`}
        </pre>
        <p>
          配置中保存的密钥优先于环境变量。
          使用 <code className="inline">ghosty auth clear --provider deepseek</code> 移除已保存的密钥。
        </p>
      </>
    ),
    sources: ["#907", "#1545", "docs/CONFIGURATION.md"],
  ...
  },
  {
    ...
    a: (
      <>
        ...
        查看 <Link href="/zh/contribute" className="body-link">贡献页面</Link> 和 <a href="https://github.com/blissito/ghostycode/blob/main/CONTRIBUTING.md" className="body-link">CONTRIBUTING.md</a>。
      </>
    ),
    sources: ["CONTRIBUTING.md"],
  },
...
brew update && brew upgrade ghosty`}
        ...
        <p>
          ...
          如果镜像延迟，请从 <a href="https://github.com/blissito/ghostycode/releases" className="body-link">GitHub Releases</a> 直接下载。
        </p>
      </>
    ),
    sources: ["README.md", "#1869", "#1914"],
