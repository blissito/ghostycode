...
export const docsMcp: DocsMcpDict = {
  metaTitle: "MCP · Ghosty Docs",
  metaDescription:
    "Consume external tool servers over the Model Context Protocol, or expose Ghosty itself as an MCP server.",
  bodyClassName: "text-ink-soft leading-relaxed",
  overviewLead:
    "Ghosty can load additional tools via MCP (Model Context Protocol). MCP servers can be local stdio processes that the TUI starts, or remote URL-based servers that speak Streamable HTTP with legacy SSE fallback. A successfully connected server registers its tools into the model catalog; a failed or disabled server is never presented as an available tool.",
  overviewConfig:
    "The config file defaults to {configPath} (the legacy {legacyConfigPath} is still read when the Ghosty file is absent), overridable with {configPathOption} or {configEnvVar}. The {serversKey} key used by other clients is accepted too.",
  ...
  serverTitle: "Ghosty as an MCP server",
  serverLead:
    "{serveMcp} runs Ghosty as an stdio MCP server so other sessions (or any MCP client) can call its tools; {mcpServerCommand} is the equivalent dispatcher entrypoint. {addSelfCommand} resolves the current binary path and writes the server into your MCP config. Keep the modes distinct: {serveHttp} is the runtime HTTP/SSE API, a separate surface.",
