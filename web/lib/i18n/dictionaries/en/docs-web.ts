...
export const docsWeb: DocsWebDict = {
  metaTitle: "Browser Client · Ghosty Docs",
  metaDescription:
  ...
  troubleshootingLead:
    "If port 7878 is occupied, pass an unused --port. If the browser cannot be opened, the command exits with an error rather than leaving a reusable bootstrap capability behind; check the OS default-browser setup and start again. If the page loads but a provider is unavailable, inspect ghosty doctor and /provider — the web command does not configure or move provider credentials. If a session expired, restart ghosty web to mint a new process-local session; reusing an old bootstrap URL is expected to fail.",
