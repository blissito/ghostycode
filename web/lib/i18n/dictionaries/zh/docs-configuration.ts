...
export const docsConfiguration: DocsConfigurationDict = {
  metaTitle: "配置 · Ghosty 文档",
  metaDescription: "config.toml 的查找顺序、项目级覆盖、凭据优先级和旧版路径迁移。",
  bodyClassName: "text-ink-soft leading-[1.9] tracking-wide",
  overviewTitle: "配置",
  overviewLead:
    "Ghosty 从 ~/.ghosty/config.toml 读取配置（旧版 ~/.deepseek/config.toml 仍作为回退读取）。--config 标志和 GHOSTY_CONFIG_PATH 环境变量可以指定别的路径，两者同时设置时 --config 优先；文件加载之后再应用环境变量覆盖。",
  auditLead:
    "在 TUI 里运行 {auditCommand} 可以查看哪些文档化的键能在当前会话修改、哪些能持久化、哪些只能改文件或需要重启——改动前以它输出的“Command / reason”列为准。",
  overlayTitle: "项目级覆盖",
  overlayLead:
    "当工作区包含常规文件 <workspace>/.ghosty/config.toml 时，其中声明的安全取值会合并到全局配置之上（旧版 <workspace>/.deepseek/config.toml 在新路径缺失时仍会读取；符号链接的项目配置会被拒绝）。这让仓库可以建议模型或收紧本地安全姿态，而不动用户的全局配置。单次启动可用 --no-project-config 跳过覆盖。",
  overlayLimits:
  ...
  legacyLead:
    "Ghosty 由 DeepSeek-TUI 更名而来。为了不破坏既有安装，运行时从新的 ~/.ghosty/ 位置读取状态，但在只有旧目录存在时回退到 ~/.deepseek/，并且始终写入 ~/.ghosty/——读取带回退、写入新位置。状态目录解析集中在 crates/config/src/lib.rs 的 resolve_state_dir / ensure_state_dir 中，每一处旧路径引用都有审计过的保留决定。",
