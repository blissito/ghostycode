...
export const docsMcp: DocsMcpDict = {
  metaTitle: "MCP · Ghosty 文档",
  metaDescription:
    "通过 Model Context Protocol 消费外部工具服务器，或把 Ghosty 作为 MCP 服务器暴露。",
  bodyClassName: "text-ink-soft leading-[1.9] tracking-wide",
  overviewLead:
    "Ghosty 可以通过 MCP（Model Context Protocol）加载额外的工具。MCP 服务器可以是由 TUI 启动的本地 stdio 进程，也可以是远程 URL 服务器（Streamable HTTP，带旧版 SSE 回退）。连接成功的服务器会把工具注册进模型目录；失败或被禁用的服务器不会作为可用工具呈现给模型。",
  overviewConfig:
  ...
  serverTitle: "把 Ghosty 作为 MCP 服务器",
  serverLead:
    "{serveMcp} 会把 Ghosty 作为 stdio MCP 服务器运行，让其他会话（或任何 MCP 客户端）调用它的工具；{mcpServerCommand} 是 dispatcher 暴露的等价入口。{addSelfCommand} 会自动解析当前二进制路径并把服务器写进你的 MCP 配置。注意区分：{serveHttp} 是运行时 HTTP/SSE API，是另一种模式。",
