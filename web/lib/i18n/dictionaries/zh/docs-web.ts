...
export const docsWeb: DocsWebDict = {
  metaTitle: "浏览器客户端 · Ghosty 文档",
  metaDescription: "仅回环的内嵌浏览器客户端：一次性引导、会话 Cookie 与本地信任边界。",
  bodyClassName: "text-ink-soft leading-[1.9] tracking-wide",
  overviewTitle: "浏览器客户端",
  overviewLead:
    "{webCommand} 在 canonical 运行时 API 之上打开 Ghosty 内嵌的浏览器客户端。它是一个纯本地界面：服务器始终绑定 {loopbackHost}，无法改绑到局域网地址，也无法在关闭运行时认证的情况下运行。默认地址是 {defaultUrl}；端口冲突时用 {portExample} 换一个回环端口。Ctrl+C 停止进程，浏览器会话随之结束。",
  overviewBody:
  ...
  troubleshootingLead:
    "端口 7878 被占用时用 --port 换一个。浏览器无法打开时命令会报错退出，而不会留下可重用的引导凭证；检查系统默认浏览器设置后重新启动。页面能打开但 provider 不可用时，查 ghosty doctor 和 /provider——web 命令不配置也不迁移 provider 凭据。会话过期后重启 ghosty web 以签发新的进程本地会话；重用旧的引导 URL 本来就会失败。",
